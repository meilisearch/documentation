...
When using Meilisearch's network sharding feature, the document fetch routes (`GET` and `POST /indexes/{index_uid}/documents`) now retrieve documents from all shards by default. Set `useNetwork: false` in your request to fetch only from the local instance.

**Wildcard facet patterns in search**

Facet search now supports wildcard patterns. You can filter by patterns containing `*`, for example `dogs.*`, to match all facets under a given prefix.
