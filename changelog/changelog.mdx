...
### Search result pagination with exact hit counts

Meilisearch now supports pagination with exact hit counts using two new search parameters: `page` and `hitsPerPage`. Unlike the previous `offset` and `limit` parameters which return estimated results, these new parameters provide exhaustive `totalHits` and `totalPages` values, enabling reliable numbered page selectors in your UI.
...
You can now cancel tasks that are `enqueued` or `processing` using the new `/tasks/cancel` endpoint. This is useful when you need to interrupt a long-running task or stop a task that was enqueued by mistake.

Filter tasks by `uids`, `statuses`, `types`, `indexUids`, or by date fields (`beforeEnqueuedAt`, `afterEnqueuedAt`, `beforeStartedAt`, `afterStartedAt`).
...
### Delete finished tasks from history

A new `DELETE /tasks` endpoint lets you remove finished tasks (`succeeded`, `failed`, or `canceled`) from your task history. This helps reduce disk space usage.

Filter tasks by `uids`, `statuses`, `types`, `indexUids`, `canceledBy`, or by date fields (`beforeEnqueuedAt`, `afterEnqueuedAt`, `beforeStartedAt`, `afterStartedAt`, `beforeFinishedAt`, `afterFinishedAt`).
...
Deploy new index versions with zero downtime using the new `/swap-indexes` endpoint. This enables atomic swapping of multiple indexes, allowing you to update your search data without any downtime for clients.
...
You can now configure Meilisearch using a TOML configuration file instead of only command-line arguments. By default, Meilisearch looks for `./config.toml` in the working directory. Use the `--config-file-path` option to specify a different location.
...
### Enhanced task filtering for GET /tasks

The `GET /tasks` endpoint now supports additional filters: `uids`, `canceledBy`, `beforeEnqueuedAt`, `afterEnqueuedAt`, `beforeStartedAt`, `afterStartedAt`, `beforeFinishedAt`, and `afterFinishedAt`.

### Compressed API requests

Full support for compressed API requests using Gzip, Brotli, and Zlib.

### Indexing performance improvements

Significant improvements to indexing speed and reduction in disk space usage by Meilisearch.

### Improved default bind address

Changed the default bind address from `127.0.0.1` to `localhost`, making the server accessible via both IPv4 and IPv6.

### Apple Silicon binaries

Added native binary support for Apple Silicon devices.

### Increased concurrent readers

Maximum concurrent readers increased from 126 to 1024.

### Reduced binary size

Support for excluding specialized tokenizations when building binaries to reduce Meilisearch binary size.

### Task transparency improvements

Snapshot creation tasks are now added to the task list for better visibility into snapshot operations.

## Other

### Parameter name changes for task filtering

Several query parameter names for `/tasks` endpoints have been renamed:
...
- `invalid_task_status` renamed to `invalid_task_statuses_filter`

### Changed field name in delete operations

The `receivedDocumentIds` field in task objects has been renamed to `providedIds` when performing delete-batch actions.

### Missing master key error handling

When attempting to use `/keys` without a master key set, Meilisearch now returns the `missing_master_key` error instead of `invalid_api_key`.

### Always-present task fields

The `canceledBy` and `error` fields are now always part of the task object, even when empty.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.30.0)
...
Control how Meilisearch matches documents during search by choosing to return only documents containing all query terms.

Set `"matchingStrategy": "all"` to return only documents containing every query term:
...
The default matching strategy is `last`, which returns documents containing all query words first, followed by documents that don't contain all query words.

### Thai language support

Meilisearch now offers optimized support for Thai language processing.

### New filter operators: EXISTS and IN

Use the `EXISTS` and `IN` operators with the `filter` search parameter:

- `price EXISTS` - selects documents where the `price` field is present
- `price IN [30, 20]` - selects documents where `price` equals 30 or 20

These operators work with the existing `NOT` (`!=`) operator for more flexible filtering.
...
### Wildcard in key management

Use the `*` helper in API key actions to express multiple permissions concisely. For example, use `indexes.*` instead of listing `indexes.create`, `indexes.get`, `indexes.update`, and `indexes.delete` individually.

## Improvements

### Performance improvements for indexing

Indexing speed has been improved, especially for indexes using `filterableAttributes` and when updating or deleting documents.

### Auto-batching enabled by default

Auto-batching is now enabled by default to increase indexing speed. If you previously used the `--enable-auto-batching` flag, remove it when upgrading. The experimental configuration flags `--debounce-duration-sec`, `--max-batch-size`, and `--max-documents-per-batch` have been removed.

If you experience indexing issues, use the `--disable-auto-batching` flag or set the `MEILI_DISABLE_AUTO_BATCHING` environment variable.

## Other

### Breaking change: NOT filter operator behavior

The `!=` (`NOT`) filter operator now matches documents missing the specified attributes. This change makes the behavior consistent with the new `EXISTS` and `IN` operators.

### Note on reserved keywords in filters

Reserved keywords must be wrapped in quotation marks when used as field names or values in filters.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.29.0)
...
The smart crop algorithm finds the window(s) with the highest density of query terms within the `cropLength` limit. From these, it picks the window that contains the most query terms in order, providing better context in cropped search results.

### API key management with deterministic keys

API keys can now be created with deterministic `key` values by specifying a `uid` field at creation. The `uid` value must be a UUID v4. If omitted, Meilisearch generates it automatically.
...
- `key` values are now deterministic between instances sharing the same configuration
- Added a `name` field for human-readable API key identification
- New actions to manage API keys: `keys.get`, `keys.create`, `keys.update`, `keys.delete`
- API key `actions`, `indexes`, and `expiresAt` cannot be updated after creation

### Hebrew language support

Hebrew is now supported as a language for tokenization and segmentation.

### Customizable facet value limits

When searching with the `facets` parameter, Meilisearch returns a maximum of 100 different facet values per faceted attribute by default. This limit is now customizable via the `/indexes/{uid}/settings/faceting` route:
...
You can retrieve the current value using `GET` or reset it using `DELETE`.

### Customizable document pagination limit

Meilisearch returns a maximum of 1000 documents per search by default. This limit is now customizable via the `/indexes/{uid}/settings/pagination` route:
...
Geosearch now supports string type for `_geo`, `lat`, and `lng` fields, improving flexibility in data formatting.

### Better index listing

The `GET /indexes` endpoint now returns paginated results:
- Use `offset` and `limit` parameters to browse indexes
- Indexes are returned in the `results` field
- Response includes `limit`, `offset`, and `total` fields

### Improved document retrieval

Document retrieval has been enhanced:
- `GET /indexes/{uid}/documents` now returns paginated results in the `results` field
- The `attributesToRetrieve` parameter has been renamed to `fields`
- The `fields` parameter is now available for both `GET /indexes/{uid}/documents` and `GET /indexes/{uid}/documents/{document_uid}` endpoints
- The `displayedAttributes` setting no longer impacts `GET /indexes/{uid}/documents`

### Task filtering and pagination

Task management has been significantly improved:
- Filter tasks by `type`, `status`, and `indexUid` on `GET /tasks`
- Use comma-separated values for OR operations: `?status=enqueued,processing`
- Use `&` for AND operations: `?status=enqueued&type=indexCreation`
- Keyset pagination with `limit` and `from` parameters
- Response includes `limit`, `from`, and `next` fields

### Asynchronous dump creation

Dump creation is now an asynchronous task:
- Dumps are placed at the end of the task queue but processed with priority
- `POST /dumps` returns `202` like other asynchronous operations

## Other

### Breaking changes in search endpoint

The `/indexes/{uid}/search` endpoint has been updated:
- `nbHits` renamed to `estimatedTotalHits`
- `facetsDistribution` parameter renamed to `facets`
- `facetsDistribution` field in response renamed to `facetDistribution`
- `matches` parameter renamed to `showMatchesPosition`
- `_matchesInfo` field renamed to `_matchesPosition`
- `exhaustiveNbHits` and `exhaustiveFacetsCount` removed

### Breaking changes in index endpoints

- Index update endpoint changed from `PUT /indexes/{indexUid}` to `PATCH /indexes/{indexUid}`
- The `name` field has been removed

### Breaking changes in document endpoints

- When retrieving documents, pagination structure changed (results now in `results` field)

### Breaking changes in tasks endpoints

- `GET /indexes/:indexUid/tasks` removed (use `GET /tasks?indexUid=:indexUid` instead)
- `GET /indexes/:indexUid/tasks/:taskUid` removed
- `uid` renamed to `taskUid` in task responses
- Task property renames:
 - `documentPartial` → `documentAdditionOrUpdate`
 - `documentAddition` → `documentAdditionOrUpdate`
 - `clearAll` → `documentDeletion`

### Breaking changes in dump endpoints

- `GET /dumps/:dump_uid/status` endpoint removed (use `GET /tasks?type=dumpCreation` instead)
- `dump_already_processing` error removed

### Breaking changes in API keys endpoint

- Tenant tokens: `apiKeyPrefix` renamed to `apiKeyUid`
- `actions`, `indexes`, and `expiresAt` can no longer be updated after key creation (only `name` and `description` are updatable)
- Key listing is now paginated with `offset` and `limit` parameters
- Keys imported from older versions will have `key` and `uid` fields regenerated
- `dumps.get` action removed

### Breaking changes in settings endpoints

The HTTP verbs for settings updates have changed:
- Global settings: `PATCH /indexes/{indexUid}/settings` (was `POST`)
- Typo tolerance: `PATCH /indexes/{indexUid}/settings/typo-tolerance` (was `POST`)
- All other individual settings endpoints now use `PUT` instead of `POST`:
...
### Highlight improvements in v0.28.1

Fixed an issue where not all words were highlighted in `_formatted` when using `attributesToCrop` and `attributesToHighlight` together.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.28.0)
...
Meilisearch now fully supports nested document fields. You can use nested field locations in any API parameter accepting document attributes using dot notation.

When setting your `filterableAttributes`:
```json
{
  "filterableAttributes": [
    "person.firstname"
...
Then, when filtering during search:
```json
{
...
You can now customize typo tolerance with the new `typoTolerance` setting endpoints at `/indexes/:index_uid/settings` and `/indexes/:index_uid/settings/typo-tolerance`.

The `typoTolerance` object accepts the following properties:
...
- `minWordSizeForTypos`:
 - `oneTypo`: Customize the minimum word size to tolerate 1 typo. (Default: `5`)
 - `twoTypos`: Customize the minimum word size to tolerate 2 typos. (Default: `9`)

### Improve cropping and highlighting

You can now customize the tags used when highlighting query terms and the crop markers when cropping long field values:

- `highlightPreTag`: Tag inserted before highlighted query terms. (Default: `<em>`)
- `highlightPostTag`: Tag inserted after highlighted query terms. (Default: `</em>`)
- `cropMarker`: Crop marker inserted around the cropped field value. (Default: `...`)

Note: `cropLength` is now measured in words instead of characters.

### Add Japanese support

New language support for Japanese via the Lindera tokenizer.

### New instance options

New instance options for limiting resource usage during indexation:

- `--max-indexing-memory` (environment variable: `MEILI_MAX_INDEXING_MEMORY`): Sets the maximum RAM size during indexation
- `--max-indexing-threads` (environment variable: `MEILI_MAX_INDEXING_THREADS`): Sets the maximum number of threads available during indexation

## Improvements

### Indexation performance

Improved indexation time when inserting documents.

### Better error messages

Improved `sortableAttributes` and `filterableAttributes` error messages during search.

## Other

### Breaking changes

- `cropLength` is now measured in words instead of characters
- Typo tolerance behavior changed: a typo on the first letter now counts as 2 typos, making searches faster in these cases
- Hard limit of 1000 retrievable documents added to search endpoints (`GET/POST /indexes/:uid/search`). This is a security measure to prevent database scraping. If you need to retrieve all documents, use the `GET /indexes/:uid/documents` endpoint instead, which does not have this limit
- Docker working directory changed: you must now mount your volume at `/meili_data`
...
- Docker binary location updated: call the binary using `meilisearch` (recommended), `/meilisearch`, or `/bin/meilisearch`
...
### Bug fixes in v0.27.1

- Fixed dumps bug during import
- Fixed geosearch bug when using mixed datasets
- Fixed nested field bug when using searchable attributes

### Bug fixes in v0.27.2

- Fixed wrong internal IDs assignments that caused bugs when using autobatching

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.27.0)
...
Tenant tokens allow you to restrict access to documents based on which end-user is making the search request. This enables multi-tenant indexes where different users see different document sets.

A tenant token is a JWT that you generate by combining an API key with a set of permissions and rules for each end-user's search access.

### Auto-batching (experimental)

Auto-batching improves indexing speed by automatically grouping consecutive document additions together. Enable it with:

```bash
meilisearch --enable-auto-batching
```

This is an experimental feature and may be reworked or removed in a future release based on user feedback.

## Improvements

- Payloads with no documents are now accepted
- CSV uploads now accept `null` values for numeric fields
- Dashboard updated with new branding

## Other

### Breaking changes

- **Dump import behavior**: Importing a dump into an instance with an existing database using `--import-dump` now throws an error. Use the new `--ignore-dump-if-db-exists` or `--ignore-missing-dump` flags to control this behavior.

- **CLI flag syntax**: The following flags no longer accept `=true` or `=false` syntax and must be used as standalone flags:
 - `--no-analytics`
 - `--schedule-snapshot`
 ...
 Example: Use `meilisearch --no-analytics` instead of `meilisearch --no-analytics=true`

- **armv8 binary removed**: Use the aarch64 binary instead.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.26.0)
...
MeiliSearch now includes a comprehensive API key management system that allows you to create, manage, and restrict access to API keys.

Key capabilities:
- Create, update, delete, list, and fetch API keys
- Restrict keys to specific actions on specific indexes
- Set expiration times for keys
- Two default API keys are automatically generated: `Default Search API Key` and `Default Admin API Key`

Manage API keys via the `/keys` endpoint (requires master-key).

### Task API

The asynchronous update system has been redesigned for better clarity and user experience:

- The `update` resource has been renamed to `task`
- Tasks are now accessible as independent resources:
 - `GET /tasks` - list all tasks
 - `GET /tasks/:taskUid` - get a specific task
 - `GET /indexes/:indexUid/tasks` - list tasks for a specific index
 - `GET /indexes/:indexUid/tasks/:taskUid` - get a specific task for an index

Task object changes:
- `updateId` is now `uid`
- `status` values: `enqueued`, `processing`, `succeeded`, `failed`
- `type` is now a string (previously an object). Possible values: `indexCreation`, `indexUpdate`, `indexDeletion`, `documentsAddition`, `documentsPartial`, `documentsDeletion`, `settingsUpdate`, `clearAll`
- New `details` object contains task-specific information
- New `indexUid` field shows which index the task affects
- Timestamps renamed: `startedProcessingAt` → `startedAt`, `processedAt` → `finishedAt`
- All timestamps now use ISO 8601 format
- Task lists are returned in a `results` array, sorted by most recent first

### Asynchronous Index Operations

Creating, updating, and deleting indexes are now asynchronous operations, eliminating potential race conditions.

## Improvements

- `matches` now works with numerical values
- Improved error messages for filter parsing
- Better Hangul tokenization
- Improved tokenization by calculating character size after normalization
- Added ARM Docker image support

## Other

### Breaking Changes to API Key Authentication

- The `X-MEILI-API-KEY` header has been replaced with `Authorization: Bearer <apiKey>`
- `public` and `private` keys no longer exist - use the default API keys instead
- The `Default Search API Key` can only be used for searching (the old `public` key could also retrieve documents)

**Migration Required**: If upgrading from a version prior to v0.25.0, update your code to use the new default API keys and the `Authorization: Bearer` header format.

### Other Breaking Changes

- API key and task object lists are now returned in a `results` array
- Environment variable `MEILI_MAX_UDB_SIZE` renamed to `MEILI_MAX_TASK_DB_SIZE`
- CLI option `--max-udb-size` renamed to `--max-task-db-size`
- Dumps created prior to v0.22 are not compatible with MeiliSearch v0.25+. To migrate: load your dump into v0.24.0, create a new dump with v0.24.0, then import it into v0.25.0
- Docker image now runs as `root` user again (reverted from v0.24.0 change). Default `data.ms` location is now `/`

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.25.0)
...
- **Increased word limit per attribute**: The maximum number of words per attribute has been increased from 1000 to 65535, allowing you to index larger text fields.
- **Reduced build size**: Binary size has been optimized for smaller downloads and faster deployments.
...
Error responses have been restructured. The following field names have changed:
- `errorCode` → `code`
- `errorType` → `type`
- `errorLink` → `link`

Failed updates now return an error object with fields: `message`, `code`, `type`, and `link`.

New error types include: `invalid_request`, `auth`, and `internal`.

New error codes: `dump_not_found`, `unretrievable_document`, `dump_already_processing`, `no_space_left_on_device`, `invalid_store_file`, `index_primary_key_already_exists`, `database_size_limit_reached`, `primary_key_inference_failed`, `document_fields_limit_reached`, `invalid_api_key`

Removed or renamed error codes: `primary_key_already_present`, `missing_primary_key`, `max_fields_limit_exceeded`, `dump_already_in_progress`, `max_database_size_limit_reached`, `invalid_token`

For a complete list of error codes and messages, refer to the API specification.

**Highlighted Attributes Behavior**

The `attributesToHighlight` feature now works on both numerical and string values. In the search response, the `_formatted` object contains only strings (and arrays/objects of strings). Numerical values from the original document are converted to strings in `_formatted`, while the original document with numerical values remains available at the root of the search response.

**Docker Container User Change**

MeiliSearch now runs as the `meili` user instead of `root` in Docker containers. The default data storage path has changed from `/data.ms` to `/home/meili/data.ms`.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.24.0)
...
### Geosearch support
Filter and sort results based on geographic criteria. You can refine precision, distance, and shape of the geographical areas in which documents should be searched.

### CSV and NDJSON dataset support
You can now import data in CSV and NDJSON formats in addition to JSON.

## Improvements

### Docker image includes curl
The Docker image now includes `curl` for easier testing and debugging.

### Better memory allocation
Switched to tikv-jemallocator for improved memory management performance.

### Search performance improvements
Optimized search performance for certain query types, including better handling of phrase matches in Chinese text.

## Other

### Breaking changes - Re-indexation required
This is a breaking release that requires re-indexation of all documents. Re-indexation happens automatically when migrating from a previous version. See the [update guide](https://docs.meilisearch.com/create/how_to/updating.html#update-to-the-latest-meilisearch-version) for more information.

### Content-Type header now mandatory
It is now mandatory to set `Content-Type` headers when using API routes that accept a payload. This behavior was already present in some routes and this update enforces it consistently. If you use one of the official Meilisearch integrations, this change will not impact your code.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.23.0)
...
Sort at query time. You can now sort results by specific fields directly in your search queries, providing more control over result ordering without needing to pre-define all possible sort combinations.

Custom ranking rules now support fields containing strings, not only numbers. This allows you to create ranking rules based on text fields in addition to numeric fields.

## Other

Custom ranking rules syntax has changed:
...
The indexer has been completely rewritten to fix crashes, reduce RAM consumption, and increase indexation speed. This requires re-indexation of all your documents, which will happen automatically when you update to this version.

If you import your data via a dump, you must manually add the `sort` ranking rule to your settings. Check your current ranking rules using `GET /indexes/:uid/settings/ranking-rules`. If `sort` is not listed, add it to the 5th position using `POST /indexes/:uid/settings`.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.22.0)
...
The `filter` search parameter replaces the previous `filters` and `facetsFilter` parameters. To use filtering, add the attribute you want to filter on to the `filterableAttributes` index setting.

Filter expressions support the `=` syntax (e.g., `genre = comedy`) and can be combined as strings or nested arrays:
...
### Phrase Search
You can now perform phrase searches by wrapping words in quotes within your search query.

### Processing Update Status
A new update status, `processing`, has been added to track indexing operations.

### Dump Status Timestamps
The `/dumps/{dump_id}/status` route now includes `startedAt` and `finishedAt` fields to track dump operation timing.

## Improvements

### New Web UI
The web interface has been completely redesigned for better usability.

### Enhanced Faceting
- Facets now support numeric values
- Facet distribution works on array fields

### Search Performance
- Overall search performance has been improved
- Typo tolerance now works on the first letter
- The new default ranking rules are: `words`, `typo`, `proximity`, `attribute`, `exactness`

### Highlighting
The `attributesToHighlight` parameter now highlights matches inside arrays and objects for more comprehensive search result previews.

### Indexing & Memory
- RAM usage during indexation has been reduced
- The 200 index limit per database has been removed

### Configuration & Environment
- Users can now specify payload limit size units (e.g., Kb, Mb, Gb)
- Enhanced logging with `MEILI_LOG_LEVEL` environment variable and `--log-level` option. Debug level now outputs queries and search results
- Analytics (Amplitude and Sentry) are unified and can be disabled at once with `MEILI_NO_ANALYTICS` environment variable and `--no-analytics` flag

### Other Improvements
- `/stats` route performance has been improved
- Cropping behavior is now consistent
- Compilation time has been improved with a caching strategy

## Other

### Breaking Changes

**Index Setting Renamed**
- `attributesForFaceting` has been renamed to `filterableAttributes`

**Search Parameters Changed**
- The `filters` and `facetsFilter` search parameters have been removed. Use `filter` instead.
- The `:` syntax for filter expressions (e.g., `genre: comedy`) is deprecated. Use the `=` syntax instead.
- When using `filter` on array fields, the `!=` operator will only return results that do not contain the value.

**Field Renamed**
- `fieldsDistribution` has been renamed to `fieldDistribution`

**Environment Variable Renamed**
- `MAX_MDB_SIZE` environment variable and `--max-mdb-size` option flag have been renamed to `MAX_INDEX_SIZE` and `--max-index-size`

**Version Route Change**
- In the `/version` route, `buildDate` is now named `commitDate` and the format has changed. MeiliSearch is now reproducible.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.21.0)
...
Improvements to health check and root endpoint responses for better API consistency.

## Other

- The `/health` endpoint now returns HTTP `200` instead of `204`
- The root `/` endpoint now returns HTTP `200` when running in a production environment

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.20.0)

</Update>

<Update label="v0.19" description="2021-02-09">

I don't have any release notes content to transform into a changelog for v0.19.0. 

To create a changelog entry, please provide the actual release notes or commit messages from the v0.19.0 release that describe:
- New features added
- Improvements made to existing functionality
- Breaking changes or deprecations
- Other notable changes

Once you share that information, I'll format it according to the changelog guidelines.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.19.0)
...
The tokenizer has been updated with improved language processing capabilities for better search results across different languages.

## Improvements

### Increased default payload size

The default maximum payload size has been increased to accommodate larger requests.

## Other

### Fixed setting consistency bug

Resolved an issue where index settings could become inconsistent in certain scenarios.

### Fixed attributes to retrieve bug

Corrected a bug affecting the retrieval of specific document attributes.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.18.0)

</Update>

<Update label="v0.17" description="2020-12-07">

(No relevant changelog content - this release contains only bug fixes, internal maintenance, CI/CD changes, and documentation updates)

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.17.0)
...
Index auto-creation on document push
Indexes are now automatically created when you push documents to a non-existent index, eliminating the need to manually create indexes first.

## Improvements

Sorted displayedAttributes and facetDistribution
The `displayedAttributes` and `facetDistribution` in search responses are now returned in sorted order for consistency and predictability.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.16.0)
...
**Dumps**
A new dump feature has been introduced, allowing you to create snapshots of your Meilisearch instance for backup and migration purposes.

## Improvements

**Empty query handling**
Empty queries are now treated as placeholder searches, providing more consistent search behavior when no query string is provided.

**Updated dependencies**
actix-web dependency has been updated to version 3.0.0 for improved stability and performance.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.15.0)

</Update>

<Update label="v0.14" description="2020-08-31">

No relevant changelog content found. The listed items are bug fixes and internal improvements that don't introduce new features or significant behavioral changes for end users.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.14.0)
...
### Placeholder search
Search queries can now include placeholders to find documents matching partial patterns.

### Displayed and searchable attributes wildcard
You can now use wildcards (`*`) to specify all attributes as displayed or searchable attributes, simplifying configuration for documents with many fields.

### Error codes in updates
Update operations now return standardized error codes, making it easier to programmatically handle and identify specific failure reasons.

## Improvements

### Database version mismatch check
Added automatic detection and reporting when the database version doesn't match the Meilisearch version, preventing compatibility issues.

### Facet distribution case handling
Fixed facet distribution to properly handle case sensitivity, ensuring consistent results.

### Unique docid handling
Improved the handling of unique document IDs to prevent edge case bugs.

### Sentry integration control
Added command-line argument to disable Sentry error tracking when needed.

### Analytics logging
Analytics data is now properly logged when analytics are enabled, providing better visibility into usage patterns.

## Other

### Removed `/sys-info` route
The system information endpoint has been removed. Use alternative methods to retrieve system information if needed.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.13.0)

</Update>

<Update label="v0.12" description="2020-06-30">

This release contains bug fixes and internal improvements but no new features or user-facing enhancements that would be included in a changelog for end users.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.12.0)
...
### Faceted Search
Added support for faceted search, allowing you to filter and categorize search results by specified fields.

### Facet Count
Added support for facet count to get the number of documents matching each facet value.

### SSL Support
Added SSL/TLS support for secure HTTPS connections to your MeiliSearch instance.

### Error Code Reporting
Added support for error code reporting to help identify and handle specific error types programmatically.

### Configurable LMDB Map Size
Added support for configuring the LMDB map size, allowing you to optimize memory usage for your dataset size.

### Max Payload Size Override
Added ability to override the maximum HTTP payload size to handle larger requests.

### Dashboard on Private Servers
The dashboard can now query private servers, making it accessible for testing and development on non-public instances.

### Telemetry
Added telemetry to help improve MeiliSearch by collecting usage information.

## Improvements

### HTTP Framework Migration
Migrated the HTTP framework from Tide to Actix-web for improved performance and reliability.

### Terminal Greeting
Improved the terminal greeting message displayed when MeiliSearch starts.

### Highlight Alignment
Fixed misalignment issues in search result highlights.

### URL Path Handling
Fixed issues with URL trailing slashes and double slashes in API requests.

### Content-Type Handling
Fixed the server to properly accept all Content-Type headers by default.

### Error Messages from Deserialization
Improved error reporting by returning detailed error messages from Serde when deserialization errors occur.

### Dashboard Accessibility
Fixed the NormalizePath middleware to ensure the dashboard is properly accessible at all URL paths.

### Settings Consistency
Improved consistency of settings handling across the application.

### Facet Cache on Document Update
Fixed facet cache to properly update when documents are modified.

## Other

### Stats Field Rename
Renamed `fieldsFrequency` to `fieldsDistribution` in the stats endpoint to better reflect its purpose.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.11.0)
...
Filtering capabilities have been expanded with support for floating point numbers and arrays of strings, allowing more precise data filtering.

### Number of Hits in Search Results
Search results now include the total number of hits, making it easier to understand result set sizes.

### Aligned Crop in Search Results
Search results now support aligned crop functionality for better content preview display.

### Nested Value Support
Added support for nested null, boolean, and sequence values in documents, enabling more complex data structures.

### Wildcard Support for Attributes
Wildcard patterns are now supported when specifying attributes to retrieve, highlight, and crop in search requests.

### '@' Character as Tokenizer Separator
The '@' character is now recognized as a tokenizer separator for better text processing.

## Improvements

### Better Web Interface Content Display
Content displayed in the web interface is now properly sanitized for improved security.

### CORS Support
Fixed CORS-related issues to improve cross-origin request handling.

## Other

### ARMv7 and ARMv8 Binary Distribution
ARMv7 and ARMv8 binaries are now published with releases for broader platform support.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.10.0)

</Update>

<Update label="v0.9" description="2020-03-19">

# v0.9.0

This release contains significant API changes. Please review the changes carefully when upgrading.

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.9.0)
...
The HTTP server now accepts updates without waiting for processing to complete. You can send updates and receive a response after body parsing, rather than waiting for the full update to be processed.

## Improvements

**Reduced Runtime Dependencies**
Removed the `libssl` runtime dependency, making MeiliSearch easier to install without requiring system-level SSL libraries.

**Updated Output Format**
The updates display format has been changed for better clarity and consistency.

## Other

**Breaking Changes**

- Port changed from 8080 to 7700 to avoid conflicts with common TCP servers
- HTTP APIs have been redesigned in this major release; see the documentation for migration details

[Find more information on GitHub](https://github.com/meilisearch/meilisearch/releases/tag/v0.8.0)
