...
A new experimental `foreignKeys` feature allows you to hydrate documents with data from other indexes, enabling you to create relationships between indexes.
...
Then configure foreign keys in your index settings at `/indexes/{index_uid}/settings`:
...
When you have a document with foreign document IDs:
...
The search response will automatically hydrate it with the related documents:
...
Note: This feature does not yet support remote sharding environments.

### Server-Sent Events (SSE) streaming improvements

Added the `X-Accel-Buffering: no` header to the `POST /chats/{workspace_uid}/chat/completions` endpoint when streaming mode is enabled. This prevents proxy response buffering for real-time chat completions.

## Improvements

### Performance

Fixed a significant memory leak in the indexation pipeline that was introduced in v1.12. If you've observed Meilisearch using progressively more memory over time, this issue has been resolved.

Restored task deletion performance to v1.38.0 levels after a regression introduced in v1.38.1, while ensuring no race conditions occur during updates.
