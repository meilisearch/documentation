...
The query UID links events to specific searches:

```javascript
// Get query UID from response metadata
const response = await index.search('headphones');
const queryUid = response.metadata?.queryUid;

// Send click event linked to that search
...
const queryUid = results.metadata?.queryUid;
...
```jsx
function Hit({ hit, position, queryUid, userId }) {
  const trackClick = () => {
