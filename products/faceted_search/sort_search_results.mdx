...
<Tip>
Sorting at search time can be particularly effective when combined with [placeholder searches](/reference/api/indexes/search-with-post#placeholder-search).
</Tip>
...
3. Update Meilisearch's [ranking rules](/products/full_text_search/ranking_rules) (optional)
...
When users sort results at search time, [Meilisearch's ranking rules](/products/full_text_search/ranking_rules) are set up so the top matches emphasize relevant results over sorting order. You might need to alter this behavior depending on your application's needs.
...
After configuring `sortableAttributes`, you can use the [`sort` search parameter](/reference/api/indexes/search-with-post#sort) to control the sorting order of your search results.
...
There is a lot of overlap between sorting and configuring [custom ranking rules](/products/full_text_search/custom_ranking_rules), as both can greatly influence which results a user will see first.
