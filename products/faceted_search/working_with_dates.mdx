...
Once all documents in your dataset have a date field, [index your data](/reference/api/documents/overview#add-or-replace-documents) as usual. The example below adds a <a id="downloadVideogames" href="/assets/datasets/videogames.json" download="videogames.json">videogame dataset</a> to a `games` index:

<CodeSamplesDateGuideIndex1 />

## Filtering by date

To filter search results based on their timestamp, add your document's timestamp field to the list of [`filterableAttributes`](/reference/api/settings/get-filterableattributes#update-filterable-attributes):
...
To sort search results chronologically, add your document's timestamp field to the list of [`sortableAttributes`](/reference/api/settings/get-sortableattributes#update-sortable-attributes):
