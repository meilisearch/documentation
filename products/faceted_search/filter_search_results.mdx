...
`filter` is a search parameter you may use at search time. `filter` accepts [filter expressions](/products/faceted_search/filter_expression_reference) built using any attributes present in the `filterableAttributes` list.
...
<Warning>
[Synonyms](/products/full_text_search/synonyms) don't apply to filters. Meaning, if you have `SF` and `San Francisco` set as synonyms, filtering by `SF` and `San Francisco` will show you different results.
</Warning>
