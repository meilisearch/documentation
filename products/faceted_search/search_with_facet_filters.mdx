...
In Meilisearch, facets are a specialized type of filter. This guide shows you how to configure facets and use them when searching a database of books. It also shows you how to get facet distribution data and search facet values.
...
You can also search for facet values with the [facet search endpoint](/reference/api/facet-search/overview):
...
You can further refine results using the `q`, `filter`, and `matchingStrategy` parameters. [Learn more about them in the API reference.](/reference/api/facet-search/overview)
