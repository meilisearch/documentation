...
<CardGroup cols={2}>
  <Card title="Batch queries" icon="layer-group" href="/reference/api/multi-search/overview">
...
```mermaid
graph LR
    A[Single Request] --> B["/multi-search"]
    B --> C[Query 1: products]
    B --> D[Query 2: reviews]
    B --> E[Query 3: articles]
    C --> F{Federation?}
    D --> F
    E --> F
    F -->|No| G[Separate Results]
    F -->|Yes| H[Merged & Ranked]
...
1. **Send** multiple queries in a single HTTP request to `/multi-search`
2. **Execute** each query against its target index in parallel
3. **Return** separate results per query, or merge into a single ranked list with federation
...
<CardGroup cols={2}>
  <Card title="Multi-search API" icon="code" href="/reference/api/multi-search/overview">
