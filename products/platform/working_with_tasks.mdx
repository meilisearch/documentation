...
[Many Meilisearch operations are processed asynchronously](/products/platform/asynchronous_operations) in a task. Asynchronous tasks allow you to make resource-intensive changes to your Meilisearch project without any downtime for users.
...
If the task is still `enqueued` or `processing`, wait a few moments and query the database once again. You may also [set up a webhook listener](/reference/api/webhooks/overview).
...
You have seen what happens when an API request adds a task to the task queue, and how to check the status of a that task. Consult the [task API reference](/reference/api/tasks/overview) and the [asynchronous operations explanation](/products/platform/asynchronous_operations) for more information on how tasks work.
