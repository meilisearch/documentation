...
Each search rule object may contain a single `filter` key. This `filter`'s value must be a [filter expression](/products/faceted_search/filter_expression_reference):
...
Consult the search API reference for [more information on Meilisearch filter syntax](/reference/api/indexes/search-with-post#filter).
</Warning>
...
Query the [get one API key endpoint](/reference/api/keys/overview) to obtain an API key's UID.

The UID must indicate an API key with access to [the search action](/reference/api/keys/overview#actions). A token has access to the same indexes and routes as the API key used to generate it.

<Warning>
If an API key expires or is deleted, any tenant tokens created with it will become invalid. Always generate tokens on your application's backend.
</Warning>
...
<Warning>
The only way to revoke a token without an expiry date is to [delete](/reference/api/keys/overview#delete-a-key) its parent API key.
</Warning>
