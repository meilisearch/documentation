...
A [task object](/reference/api/tasks/overview#task-object) includes data not present in the original request, such as when the request was enqueued, the type of request, and an error code when the task fails:
...
For a comprehensive description of each task object field, consult the [task API reference](/reference/api/tasks/overview).

#### Summarized task objects

When you make an API request for an asynchronous operation, Meilisearch returns a [summarized version](/reference/api/tasks/overview#summarized-task-object) of the full `task` object.
...
Use the summarized task's `taskUid` to [track the progress of a task](/reference/api/tasks/overview#get-one-task).
...
`succeeded`, `failed`, and `canceled` tasks are finished tasks. Meilisearch keeps them in the task database but has finished processing these tasks. It is possible to [configure a webhook](/reference/api/webhooks/overview) to notify external services when a task is finished.
...
- [`dumpCreation`](/reference/api/tasks/overview#dumpcreation)
- [`taskCancelation`](/reference/api/tasks/overview#taskcancelation)
- [`taskDeletion`](/reference/api/tasks/overview#taskdeletion)
- [`indexSwap`](/reference/api/tasks/overview#indexswap)
- [`snapshotCreation`](/reference/api/tasks/overview#snapshotcreation)
...
<Warning>
When the task queue reaches its limit (about 10GiB), it will throw a `no_space_left_on_device` error. Users will need to delete tasks using the [delete tasks endpoint](/reference/api/tasks/overview#delete-tasks) to continue write operations.
</Warning>
...
1. Meilisearch creates a task, puts it in the task queue, and returns a [summarized `task` object](/products/platform/asynchronous_operations#summarized-task-objects). Task `status` set to `enqueued`
...
Meilisearch processes tasks in batches, grouping tasks for the best possible performance. In most cases, batching should be transparent and have no impact on the overall task workflow. Use [the `/batches` route](/reference/api/batches/overview) to obtain more information on batches and how they are processing your tasks.

### Canceling tasks

You can cancel a task while it is `enqueued` or `processing` by using [the cancel tasks endpoint](/reference/api/tasks/overview#cancel-tasks). Doing so changes a task's `status` to `canceled`.
...
[Finished tasks](#task-status) remain visible in [the task list](/reference/api/tasks/overview#get-tasks). To delete them manually, use the [delete tasks route](/reference/api/tasks/overview#delete-tasks).
...
Suppose you add a new document to your instance using the [add documents endpoint](/reference/api/documents/overview#add-or-replace-documents) and receive a `taskUid` in response.

When you query the [get task endpoint](/reference/api/tasks/overview#get-one-task) using this value, you see that it has been `enqueued`:
...
If the task had been [canceled](/reference/api/tasks/overview#cancel-tasks) while it was `enqueued` or `processing`, it would have the `canceled` status and a non-`null` value for the `canceledBy` field.

After a task has been [deleted](/reference/api/tasks/overview#delete-tasks), trying to access it returns a [`task_not_found`](/reference/errors/error_codes#task_not_found) error.
