...
When using the [`showRankingScore` search parameter](/reference/api/indexes/search-with-post#ranking-score), Meilisearch adds a global ranking score field, `_rankingScore`, to each document. The `_rankingScore` is between `0.0` and `1.0`. The higher the ranking score, the more relevant the document.
