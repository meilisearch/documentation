...
There are two types of ranking rules in Meilisearch: [built-in ranking rules](/products/full_text_search/ranking_rules) and custom ranking rules. This article describes the main aspects of using and configuring custom ranking rules.
...
You can add this rule to the existing list of ranking rules using the [update settings endpoint](/reference/api/settings/overview#update-settings) or [update ranking rules endpoint](/reference/api/settings/get-rankingrules#update-ranking-rules).
...
Meilisearch allows users to define [sorting order at query time](/products/faceted_search/sort_search_results) by using the [`sort` search parameter](/reference/api/indexes/search-with-post#sort). There is some overlap between sorting and custom ranking rules, but the two do have different uses.
