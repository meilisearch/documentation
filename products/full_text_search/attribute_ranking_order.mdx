...
If you manually configure [the searchable attributes list](/products/full_text_search/displayed_searchable_attributes#the-searchableattributes-list), attributes that appear early in the array are more important when calculating search result relevancy.
...
<Note>
The `attribute` rule's position in [`rankingRules`](/products/full_text_search/ranking_rules) determines how the results are sorted. Meaning, **if `attribute` is at the bottom of the ranking rules list, it will have almost no impact on your search results.**
</Note>
