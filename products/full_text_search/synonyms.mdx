...
If multiple words have an equivalent meaning in your dataset, you can [create a list of synonyms](/reference/api/settings/get-synonyms#update-synonyms). This will make your search results more relevant.
...
With the new settings, searching for `great` returns 595 results and `fantastic` returns 423 results. This is due to various factors like [typos](/products/full_text_search/typo_tolerance_settings#minwordsizefortypos) and [splitting the query](/products/full_text_search/query_processing#split-queries) to find relevant documents. The search for `great` will allow only one typo (for example, `create`) and take into account all variations of `great` (for instance, `greatest`) along with `fantastic`.
...
However, if you search for `iphone`, documents containing `phone` will be ranked lower in the results due to [the typo rule](/products/full_text_search/ranking_rules).

### Example

To create a one-way synonym list, this is the JSON syntax that should be [added to the settings](/reference/api/settings/get-synonyms#update-synonyms).
...
To create a mutual association between four words, this is the JSON syntax that should be [added to the settings](/reference/api/settings/get-synonyms#update-synonyms).
...
Meilisearch treats multi-word synonyms as [phrases](/reference/api/indexes/search-with-post#phrase-search).
