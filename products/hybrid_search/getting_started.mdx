...
First, create a new Meilisearch project. If this is your first time using Meilisearch, follow the [quick start](/getting_started/cloud_quick_start) then come back to this tutorial.
...
For practical information on implementing AI-powered search with other services, consult our [guides section](/guides/ai_embedders/providers/openai). There you will find specific instructions for embedders such as [LangChain](/guides/ai_embedders/langchain) and [Cloudflare](/guides/ai_embedders/providers/cloudflare).

For more in-depth information, consult the API reference for [embedder settings](/reference/api/settings/get-embedders) and [the `hybrid` search parameter](/reference/api/indexes/search-with-post#hybrid-search).
