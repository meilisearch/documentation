...
Next, use [the `/documents` endpoint](/reference/api/documents/overview?utm_campaign=vector-search&utm_source=docs&utm_medium=vector-search-guide) to upload vectorized documents. Place vector data in your documents' `_vectors` field:

```sh
curl -X POST -H 'content-type: application/json' \
"${MEILISEARCH_URL}/indexes/products/documents" \
--data-binary '[
...
`vector` can be used together with [other search parameters](/reference/api/indexes/search-with-post?utm_campaign=vector-search&utm_source=docs&utm_medium=vector-search-guide), including [`filter`](/reference/api/indexes/search-with-post#filter) and [`sort`](/reference/api/indexes/search-with-post#sort):
