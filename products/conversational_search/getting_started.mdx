...
- A [secure](/products/platform/basic_security) Meilisearch >= v1.15.1 project
...
Meilisearch Cloud projects automatically include a "Default Chat API Key" with `chatCompletions` and `search` permissions on all indexes. Check if you have the key using:
...
- `documentTemplate` defines the document data Meilisearch sends to the AI provider. This template outputs all searchable fields in your documents, which may not be ideal if your documents have many fields. Consult the best [document template best practices](/products/hybrid_search/document_template_best_practices) article for more guidance
...
`prompts.system` gives the conversational search bot the baseline context of your users and their questions. [The `prompts` object accepts a few other fields](/products/conversational_search/chat_tooling_reference#prompts) that provide more information to improve how the agent uses the information it finds via Meilisearch. In real-life scenarios filling these fields would improve the quality of conversational search results.
...
- `tools` sets up two optional but highly [recommended tools](/products/conversational_search/chat_tooling_reference):
...
const client = new OpenAI({
  baseURL: process.env.MEILISEARCH_URL + '/chats/WORKSPACE_NAME',
  apiKey: 'PROVIDER_API_KEY',
});
...
1. Enable experimental features (see setup instructions above)
...
- Use the "Default Chat API Key" or create a key with `chatCompletions` permission
- Don't use search or admin API keys for chat endpoints
- Find your chat key: `curl "${MEILISEARCH_URL}/keys" -H "Authorization: Bearer ${MEILISEARCH_API_KEY}"`
