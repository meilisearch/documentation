...
2. **Generate the Mintlify-ready file** (reads `assets/engine-release-assets/meilisearch-openapi.json`, injects code samples from this repo and SDK repos, cleans null descriptions; writes `assets/engine-release-assets/meilisearch-openapi-mintlify.json`):
...
The Cloud API reference (at the bottom of the Reference tab) is generated by Mintlify directly from `assets/cloud-assets/meilisearch-cloud-openapi.yaml`. This file is fetched by `npm run fetch-assets` from the private [meilisearch-cloud](https://github.com/meilisearch/meilisearch-cloud) repository (`api/swagger/v2/swagger.yaml` on `main`), so a token with access to that repository is required. Unlike the engine API reference, the pages are autogenerated from the spec: there is no page list to maintain in `config/navigation.json`.
