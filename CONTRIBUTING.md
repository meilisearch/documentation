...
The Management API reference (the Meilisearch Cloud API, at the bottom of the Reference tab) is generated by Mintlify from `assets/cloud-assets/meilisearch-cloud-openapi-mintlify.yaml`. That file is produced by `generate-mintlify-openapi-file` (without code samples) from `assets/cloud-assets/meilisearch-cloud-openapi.yaml`, which is fetched by `npm run fetch-assets` from the private [meilisearch-cloud](https://github.com/meilisearch/meilisearch-cloud) repository (`api/swagger/v2/swagger.yaml` on `main`), so a token with access to that repository is required. Unlike the engine API reference, the pages are autogenerated from the spec: there is no page list to maintain in `config/navigation.json`.
