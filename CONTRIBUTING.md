...
1. **Fetch the assets** (writes the latest OpenAPI and error code files from the Meilisearch release in `assets/engine-release-assets/`, and the Meilisearch Cloud OpenAPI spec in `assets/cloud-assets/`):

```bash
npm run fetch-assets
```

Set `GITHUB_PAT` or `GH_TOKEN` to fetch the Meilisearch Cloud OpenAPI spec (private repository, the fetch is skipped with a warning otherwise) and for higher API rate limits.

2. **Generate the Mintlify-ready files** (reads the given OpenAPI file, cleans null descriptions, and with `--with-code-samples` injects code samples from this repo and SDK repos; writes the result next to the source file with a `-mintlify` suffix):

```bash
# Engine (with code samples): writes assets/engine-release-assets/meilisearch-openapi-mintlify.json
npm run generate-mintlify-openapi-file -- --with-code-samples assets/engine-release-assets/meilisearch-openapi.json

# Cloud (no code samples): writes assets/cloud-assets/meilisearch-cloud-openapi-mintlify.yaml
npm run generate-mintlify-openapi-file -- assets/cloud-assets/meilisearch-cloud-openapi.yaml
```
3. **Generate the error code mdx file** (reads `assets/engine-release-assets/meilisearch-error-codes.json` and outputs `reference/errors/error_codes.mdx`):
...
## Meilisearch Cloud API reference

The Cloud API reference (at the bottom of the Reference tab) is generated by Mintlify from `assets/cloud-assets/meilisearch-cloud-openapi-mintlify.yaml`. That file is produced by `generate-mintlify-openapi-file` (without code samples) from `assets/cloud-assets/meilisearch-cloud-openapi.yaml`, which is fetched by `npm run fetch-assets` from the private [meilisearch-cloud](https://github.com/meilisearch/meilisearch-cloud) repository (`api/swagger/v2/swagger.yaml` on `main`), so a token with access to that repository is required. Unlike the engine API reference, the pages are autogenerated from the spec: there is no page list to maintain in `config/navigation.json`.
