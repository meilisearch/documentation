---
title: Use foreign keys for cross-index document hydration
description: Automatically enrich documents with related data from other indexes using the experimental foreign keys feature.
---
...
For example, a `movies` index can reference actors by ID. When you search for movies, Meilisearch automatically replaces the actor IDs with full actor documents from the `actors` index.
...
- At least two indexes (one main index and one related index)
