...
Meilisearch looks up synonyms using normalized search terms. [Typo tolerance](/capabilities/full_text_search/relevancy/typo_tolerance_settings) does not apply to this lookup.
...
After a reindex, `S.W.A.T.` counts as one word. A mutually associated synonym between `S.W.A.T.` and `swat` now resolves in both directions.
