...
<Tip>
For promoting, pinning, and boosting specific documents so they are displayed more prominently than other search results, use [search rules](/capabilities/search_rules/overview). They pin selected documents at fixed positions in search results when query- or time-based conditions match.
</Tip>
