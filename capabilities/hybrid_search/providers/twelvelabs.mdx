...
- The `multi_input` input type accepts text, media, or both in the same request shape. This lets a single `request` template serve every fragment: Meilisearch injects the fragment into the `multi_input` field.
