...
- `documentTemplate`: a [Liquid template](/capabilities/hybrid_search/getting_started) that converts your documents into text for embedding. Keep it short and include only the most important fields
...
Replace `MEILISEARCH_URL` with the address of your Meilisearch project, `INDEX_NAME` with your index name, `MEILISEARCH_API_KEY` with your Meilisearch API key, and `OPEN_AI_API_KEY` with your [OpenAI API key](https://platform.openai.com/api-keys).
