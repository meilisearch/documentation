...
- Two embedding providers that produce vectors with the same number of dimensions
...
**Compatible models**: for coherent search results, both embedders should use the same underlying model or models trained to produce compatible vector spaces. Using unrelated models (for example, one trained on English text and another on multilingual data) will produce poor search quality because the vector spaces will not align.
