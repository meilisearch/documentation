...
When a task completes, Meilisearch sends an HTTP POST request to your configured URL. The request body is [ndjson](https://jsonlines.org/) (newline-delimited JSON), with one task object per line matching the format returned by the [Tasks API](/reference/api/tasks/get-task).
