...
| Feature | Description |
|---------|-------------|
| **MCP server** | Expose your Meilisearch index through a Model Context Protocol (MCP) server for use with AI agents and tools |
...
| **"CONTAINS" filter** | Adds the `CONTAINS` filter operator to match documents where the specified attribute includes a specific substring |
...
<Note>
  This list changes over time. New features are added, and others are eventually promoted to stable and removed from the experimental list. Treat this table as a snapshot rather than an exhaustive reference, and always check your project's **Project Settings > General** in the Cloud UI for the current set.
</Note>
