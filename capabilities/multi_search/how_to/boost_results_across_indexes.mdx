...
Each query in a federated multi-search request can include a `federationOptions` object with a `weight` property. The weight is a floating-point number that multiplies the relevancy score of results from that query:
