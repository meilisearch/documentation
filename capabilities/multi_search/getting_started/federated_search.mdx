...
`mergeFacets` is an object. Its `maxValuesPerFacet` property sets the maximum number of facet values Meilisearch returns per facet after merging distributions from all indexes. If you do not set it, Meilisearch uses the default limit from the index settings.
