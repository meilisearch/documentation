...
import CodeSamplesUpdateExperimentalFeaturesContains1 from '/snippets/generated-code-samples/code_samples_update_experimental_features_contains_1.mdx';
...
<CodeSamplesUpdateExperimentalFeaturesContains1 />
</Note>
...
The following expression returns all deals where the related company is in the Technology industry:

<CodeGroup>

```
_foreign(company, industry = "Technology")
```
...
```
_foreign(company, industry = "Technology" AND founded_year >= 2000)
```

</CodeGroup>

For detailed examples and use cases, see the [Foreign filters guide](/capabilities/filtering_sorting_faceting/advanced/filtering_by_joined_data).
