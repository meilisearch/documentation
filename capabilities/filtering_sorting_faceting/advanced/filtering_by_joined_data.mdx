...
curl -X POST "http://localhost:7700/indexes/tasks/search" \
  ...
  -d '{
    "q": "project",
    "filter": "_foreign(assigned_user, role = \"admin\")"
  }'
```
...
curl -X POST "http://localhost:7700/indexes/orders/search" \
  ...
  -d '{
    "q": "laptop",
    "filter": "_foreign(customer, tenant_id = \"tenant_123\")"
  }'
```
...
curl -X POST "http://localhost:7700/indexes/stores/search" \
  ...
  -d '{
    "q": "",
    "filter": "_foreign(location, country = \"USA\") AND _foreign(category, name = \"Electronics\")"
  }'
```
