...
You can further scope the results by combining `facetQuery` with `q`, `filter`, and `matchingStrategy`. See the [facet search API reference](/reference/api/facet-search/search-for-facet-values) for the full list of parameters.
