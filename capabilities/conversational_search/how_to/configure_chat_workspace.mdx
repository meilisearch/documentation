...
Azure OpenAI requires additional fields compared to other providers:
...
The `searchParameters` object lets you control how the LLM searches each index. This is useful for enabling hybrid search, limiting results, or sorting:
...
|-----------|------|-------------|
| `hybrid` | object | Enable hybrid search with `embedder` (required) and `semanticRatio` (0.0 for keyword, 1.0 for semantic) |
