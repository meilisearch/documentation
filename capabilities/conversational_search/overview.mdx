...
{/* TODO: recommended alternative getting started article */}

### Chat completions API (fallback)

The [`/chats` route](/reference/api/chats/request-a-chat-completion) consolidates retrieval, context management, and generation into a single endpoint. Use it only if you need to call the Chats API directly.

See the [Chats API fallback](/capabilities/conversational_search/advanced/chats_api_fallback) guide for setup, chat patterns, streaming, and tools. Consult the [chat completions API reference](/reference/api/chats/request-a-chat-completion) for the full list of supported parameters.
