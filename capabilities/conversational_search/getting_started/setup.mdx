...
Your project comes with a single default workspace named `cloud`. Configure it by sending a `PATCH` request to `/chats/cloud/settings`:

<CodeGroup>

```bash OpenAI
curl \
  -X PATCH 'MEILISEARCH_URL/chats/cloud/settings' \
  -H 'Authorization: Bearer MEILISEARCH_KEY' \
  -H 'Content-Type: application/json' \
  --data-binary '{
    "source": "openAi",
...
If your use case requires multiple workspaces, [contact us](https://www.meilisearch.com/contact). This limit may change in the future.
