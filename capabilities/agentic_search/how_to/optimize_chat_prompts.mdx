...
The system prompt (set through [workspace settings](/capabilities/agentic_search/advanced/chats_api_fallback#workspace-settings)) defines the LLM's overall behavior. Beyond basic [guardrails](/capabilities/agentic_search/how_to/configure_guardrails), you can shape response quality with specific instructions.
...
Each index has chat-specific settings that control how documents are prepared for the LLM. Configure these through the [index chat settings](/capabilities/agentic_search/advanced/chats_api_fallback#index-chat-settings) endpoint:
...
<CardGroup cols={2}>
  ...
  <Card title="Configure index chat settings" href="/capabilities/agentic_search/advanced/chats_api_fallback#index-chat-settings">
