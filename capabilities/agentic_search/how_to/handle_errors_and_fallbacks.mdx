...
description: Build resilient conversational search with the AI SDK by handling LLM failures, empty results, rate limiting, and providing meaningful fallback responses.
---

Conversational search built with [`@meilisearch/ai-sdk`](/capabilities/agentic_search/getting_started) involves multiple systems: Meilisearch, an LLM provider, and your application. Any of these can fail. This guide covers common failure modes and how to handle them gracefully.
...
async function chat(prompt: string) {
  try {
    ...
    return { role: "assistant", content: "Something went wrong. Please try rephrasing your question." };
  }
}
```
...
When conversational search fails, fall back to a standard keyword or hybrid search using [`meilisearch-js`](/getting_started/sdks/javascript). This ensures users still get results:
...
When using the experimental [Chats completions API](/capabilities/agentic_search/advanced/chats_api_fallback), Meilisearch forwards HTTP status codes from your LLM provider on the completions endpoint, and you detect empty results by parsing the `_meiliSearchSources` tool call. See [handle errors and fallbacks with the Chats API](/capabilities/agentic_search/advanced/chats_api_fallback#handle-errors-and-fallbacks-with-the-chats-api) for implementation.
