...
When the LLM cannot find relevant documents, it may hallucinate an answer or give a vague response. Use [guardrails](/capabilities/agentic_search/how_to/configure_guardrails) in your system prompt to handle this:
...
function handleSources(toolCall) {
  ...
  if (!args.documents || Object.keys(args.documents).length === 0) {
...
<CardGroup cols={2}>
  ...
  <Card title="Stream chat responses" href="/capabilities/agentic_search/advanced/chats_api_fallback#streaming">
