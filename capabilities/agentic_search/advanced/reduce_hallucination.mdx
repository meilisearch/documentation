...
Meilisearch Cloud provides built-in guardrail options for the experimental [Chats completions API](/capabilities/agentic_search/advanced/chats_api_fallback) through the workspace settings. These guardrails work by injecting carefully crafted instructions into the system prompt to guide the model's behavior.
...
Configure guardrails through the [chat workspace settings](/capabilities/agentic_search/advanced/chats_api_fallback#workspace-settings) or the Meilisearch Cloud UI. Available guardrails include:
...
For detailed configuration examples, see the [Configure guardrails](/capabilities/agentic_search/how_to/configure_guardrails) guide.
...
<CardGroup cols={2}>
  ...
  <Card title="Configure a chat workspace" href="/capabilities/agentic_search/advanced/chats_api_fallback#workspace-settings">
