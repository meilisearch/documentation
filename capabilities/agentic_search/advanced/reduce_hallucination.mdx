...
Meilisearch Cloud provides built-in guardrail options for the experimental [Chat completions API](/agentic_search/advanced/chats_api_fallback) through the workspace settings. These guardrails work by injecting carefully crafted instructions into the system prompt to guide the model's behavior.
