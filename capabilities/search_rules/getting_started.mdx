...
In the **Conditions** block, click either on the **Query** combobox or **Time** combo box to configure the conditions. Every condition you add is combined with `AND`, so the rule fires only when all conditions are true at the same time.
