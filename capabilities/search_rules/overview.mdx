...
Search rules apply pinning or relevancy scaling actions to documents when specific conditions match.

Pinning actions pin selected documents at fixed positions in search results.
Pinning runs on top of organic search: Meilisearch still ranks results as usual, then inserts the pinned documents at the positions you asked for.

Relevancy scaling actions can change the natural rank of documents in the results by increasing or decreasing their relevancy. The returned documents are still organic, but their order may change.
...
```mermaid
flowchart LR
    Q[Search query] --> R[Matching rules]
    R --> P[Resolved pinned documents]
    R --> S[Resolved scaled documents]
    Q --> O[Scaled Organic results]
    S --> O
...
1. Computes the scaled organic results from the normal results and the scaled documents
...
A pinned document can appear even if it does not match the query text, but filters still apply: if a pinned document does not satisfy the current filters, Meilisearch drops it.

## Key concepts

A rule combines three parts:

- **Conditions** decide when the rule fires. Query conditions match for emptiness of the search query, or for words contained in the search query. Time conditions activate the rule during a window between a start and end timestamp. Filter conditions fire when specific facet values are selected in a filter. If multiple conditions are provided, they must all be met for the rule to apply.
- **Actions** decide what the rule does, such as pinning or relevancy scaling.
- **Precedence** decides which rule wins when several match at once. Lower numeric values take precedence over higher ones. Omitting precedence treats the rule as the last precedence, meaning it will be considered after all rules that define a precedence.
