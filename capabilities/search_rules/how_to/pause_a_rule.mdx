...
Because the `PATCH` route is an upsert that only modifies the fields you send, this request leaves every other field untouched. The response returns `202 Accepted` with the corresponding task for the asynchronous update.
...
When a rule is reactivated, it fires again from the next search request on, as soon as the corresponding asynchronous task has been processed.
...
- **Precedence interactions**: while a rule is paused, it does not participate in precedence comparisons. A late-precedence rule that would normally lose to the paused rule can now fire. Double-check after pausing an early-precedence rule.
