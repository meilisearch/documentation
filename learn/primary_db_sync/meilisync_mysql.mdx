...
Though Meilisearch is a database, it is optimized for search and underperforms when used as a primary data store. It is highly recommended you use an external database to store your data and periodically synchronise it with Meilisearch to ensure comprehensive and accurate search results.

This guide uses [`meilisync`](https://github.com/long2ice/meilisync) to keep Meilisearch up to date with a MySQL database.
...
First, create a `docker-compose.yml` file in your Docker project directory. Open the file with a text editor and add the following content to it:
...
If your project already has a `docker-compose.yml`, add the `meilisync` settings to the existing file's `services` field.
...
Then, open your command-line console and run the following command to install `meilisync` in your machine:

```sh
pip install meilisync
```

Once `pip` is done, run `meilisync`:
...
The first step is informing `meilisync` where and how it should connect to your Meilisearch instance. 
...
To use `meilisync` with a MySQL database, you must set your binary log format to `ROW`.
