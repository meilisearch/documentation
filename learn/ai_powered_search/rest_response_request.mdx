...
In Mistral, `model` must be a string specifying a valid Mistral model. Update your embedder object adding this field and its value:
...
Most providers accept either a string or an array of strings. A single string will generate one request per document in your database. An array of strings allows Meilisearch to send up to 10 documents in one request.
...
`source`, `request`, and `response` are the only embedder fields specific to REST embedders.

Like other embedders, you're likely required to supply an `apiKey` and specify the the `dimensions` of your chosen model:
