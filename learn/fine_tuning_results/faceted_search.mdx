...
Once you have configured [filterableAttributes](/reference/api/settings#filterable-attributes), you can also search for facet values with the [facet search endpoint](/reference/api/facet_search).

The following code sample searches the `genres` facet for values starting with `c`:

<CodeSamples id="faceted_search_2" />

The response contains a `facetHits`object:
