...
<Capsule intent="Facet search on numbers">
Internally, Meilisearch represents numbers as [`float64`](https://en.wikipedia.org/wiki/Double-precision_floating-point_format). Since they lack precision and can be written in different ways, Meilisearch does not support facet search on numbers. We recommend converting numbers to strings to make them searchable.
</Capsule>
...
<Capsule intent="warning">
Multi-word facets are not supported as Meilisearch only considers one term for `facetQuery`. For example, `Jane Austen` is considered a single term, searching for `Jane` will return `Jane Austen`, but searching for `Austen` will not return `Jane Austen`.
</Capsule>
