...
Dumpless upgrades are available when upgrading from Meilisearch >=v1.12 to Meilisearch >=v1.1
...
Once the snapshot is available in your snapshots folder, proceed to the next step.
...
Meilisearch should launch normally and immediately create a new `UpgradeDatabase` task. This task is processed immediately and cannot be canceled. You may follow its progress by using the `GET /tasks` endpoint to obtain its `taskUid`, then querying `GET /tasks/TASK_UID`.
...
<Capsule intent="warning">
If after the upgrade is completed and the task status is set to `failed` or Meilisearch returns internal error messages to your queries, [restart your instance from the snapshot](/learn/data_backup/snapshots#starting-from-a-snapshot) you generated during step 1.
</Capsule>
