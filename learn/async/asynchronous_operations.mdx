---
title: Tasks and asynchronous operations — Meilisearch documentation
...
`succeeded`, `failed`, and `canceled` tasks are finished tasks. Meilisearch keeps them in the task database but has finished processing these tasks. It is possible to [configure a webhook](/learn/configuration/instance_options#task-webhook-url) to notify external services when a task is finished.
