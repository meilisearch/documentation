...
A dump is a compressed file containing an export of your Meilisearch instance. Use dumps to migrate to new Meilisearch versions.

A dump contains all your indexes, documents, and settings, but in a raw unprocessed form. A dump isn't an exact copy of your database—it is closer to a blueprint that allows you to create an identical dataset.
...
Meilisearch Cloud does not support creating dumps manually. Use the Cloud interface to migrate your project to the most recent Meilisearch release.  

If you need to create a dump for reasons other than upgrading, contact the support team via the Meilisearch Cloud interface or the official Meilisaerch discord server.
