...
```
product.name CONTAINS kef
```

The negated form of the above expression can be written as:

```
product.name NOT CONTAINS kef
NOT product.name CONTAINS kef
```

<Capsule intent="note" title="Activating `CONTAINS`">
`CONTAINS` is an experimental feature. Use the experimental features endpoint to activate it:
