...
This tutorial will show you how to secure your Meilisearch. You will see how to manage your master key and how to safely send requests to the Meilisearch API using an API key.
...
Your project is now protected. Meilisearch automatically generates two API keys: `Default Search API Key` and `Default Admin API Key`. API keys are authorization tokens designed to safely communicate with the Meilisearch API. 
...
Use the `Default Admin API Key` to perform other operations, such as creating a new index:
