...
There are two steps to use tenant tokens with an official SDK: generating the tenant token, and including it in a query.

## Requirements

- a working Meilisearch project
- an application supporting authenticated users
- one of Meilisearch's official SDKs
...
**API key** must be a valid Meilisearch API key with access to [the search action](/reference/api/keys#actions). A tenant token will have access to the same indexes as the API key used when generating it. If no API key is provided, the SDK might be able to infer it automatically.
