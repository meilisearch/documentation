...
Next, you will use this tenant token to perform searches against your Meilisearch instance.

## Using the tenant token

Now that you have a tenant token, you will use it to perform searches. Like all search API keys, you can safely use it in client-side code.
...
This guide uses [Express.js](https://expressjs.com/en/starter/installing.html) to create a server. You can install `express` by running:
...
This code creates an endpoint at `http://localhost:3000/token` that accepts an `id` query parameter and returns a tenant token. You will use this endpoint to retrieve the tenant token in your front-end application.

### Making a search

This guide uses [InstantSearch.js](/learn/front_end/front_end_integration) to create a search interface. You will use CDN links to include InstantSearch.js and the Meilisearch InstantSearch.js connector in your HTML file.
