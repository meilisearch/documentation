...
<Capsule>
A _Search API key_ is a key that provides **read-only access** to your Meilisearch instance. You can retrieve it in your Meilisearch dashboard.
</Capsule>

## Use case

This guide uses the example of a customer relationship manager (CRM) application that allows users to store contacts. This CRM is a multitenant application, where each user can only access data belonging to their organization. In our example, this means that users will only be able to search contacts of their organization.
