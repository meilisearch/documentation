...
In this guide, you will see how to use JavaScript to split a single large and configure Meilisearch with a distinct attribute to prevent duplicated results.
...
`stories.json` contains two documents storing the full text of a short story in the `text` field:
...
Both these documents contain more text than Meilisearch can optimally search.

## Splitting documents

There are many ways to parse and format a dataset. In this guide, you will use Node.js to write and run a JavaScript program.
...
This script reads a JSON dataset and parses each document in it. For each paragraph in a document's `text` field, it creates a new document with a new `id` and `text` fields.
...
This dataset is now valid, but since each document effectively points to the same story, search are likely to result in duplicated search results.
...
You have seen how to split large documents to improve search relevancy. You also saw how to configure a distinct attribute to prevent duplicate results. Though this guide used JavaScript, you can replicate the process with any programming language you are comfortable using.
