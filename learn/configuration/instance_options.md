...
- Setting a [master key](/learn/security/master_api_keys.md) of at least 16 bytes is **mandatory**. If no master key is provided or if it is less than 16 bytes, Meilisearch will suggest a secure autogenerated master key
- The [search preview interface](/learn/what_is_meilisearch/search_preview.md) is disabled

`development`:

- Setting a [master key](/learn/security/master_api_keys.md) is **optional**. If no master key is provided or if it is less than 16 bytes, Meilisearch will suggest a secure autogenerated master key
...
**Default value**: `None`
**Expected value**: UTF-8 string of at least 16 bytes
...
Providing a master key is mandatory when `--env` is set to `production`. If none is given, or it is less than 16 bytes, Meilisearch will throw an error and refuse to launch. In both cases, Meilisearch will suggest a secure autogenerated master key you can use by running the following command and restarting your instance:

```
export MEILI_MASTER_KEY="SUGGESTED_MASTER_KEY"
```

If no master key is provided in a `development` environment, all routes will be unprotected and publicly accessible. Meilisearch will suggest an autogenerated secure master key if you need protection in a `development` environment.
