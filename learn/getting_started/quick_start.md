...
The previous command added documents from `movies.json` to a new index called `movies` and sets id as there primary key.
Every index has a [primary key](/learn/core_concepts/primary_key.html#primary-field), an attribute that must be shared across all documents in that index. If you attempt to add documents to an index and even a single one is missing the primary key, none of the documents will be stored.
