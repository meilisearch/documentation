...
If two indexes contain the exact same document, they will be treated as different documents and nothing will be shared between the indexes. Both documents might have different sizes depending on the [index's settings](/reference/api/settings). For example, documents in an index with `filterableAttributes` will occupy more space.
