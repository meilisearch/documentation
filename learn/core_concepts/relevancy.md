...
:::note
Please keep in mind that **the order of the results depends on the position of the `attribute` rule in the [`rankingRules`](#default-order) object.** Meaning, **if `attribute` is at the bottom of the ranking rules list, it will have no impact.**
:::
