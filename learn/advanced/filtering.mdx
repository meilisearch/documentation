...
The `IS EMPTY` operator selects documents with fields that exist, but contain empty values. The following expression only returns documents with an empty synopsis field:
...
The `IS NULL` operator selects documents with fields that exist, but contain `null` values. The following expression only returns documents with a `null` synopsis field:
