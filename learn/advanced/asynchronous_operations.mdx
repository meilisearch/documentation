...
Meilisearch does not automatically delete tasks once their status is `succeeded`, `failed`, or `canceled`. These tasks remain visible in [the task list](/reference/api/tasks#get-tasks). To delete them, use the [delete tasks route](/reference/api/tasks#delete-tasks).
