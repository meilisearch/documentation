...
When a dump is being imported, the API is not available to the task queue. As a result, no read or write operations can be performed, except importing the dump into the database at that time.
