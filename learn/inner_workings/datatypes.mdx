...
You can also create [filters](/learn/advanced/filtering). The `>`, `>=`, `<`, `<=`, and `TO` relational operators apply only to numerical values.
...
Once the above array has been flattened, it will be parsed exactly as explained in the [string example](/learn/inner_workings/datatypes#examples).
