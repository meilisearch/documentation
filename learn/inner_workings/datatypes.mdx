...
Meilisearch has no concept of subdocuments. In the previous example, the connection between the `2022-01-01` appointment and `Jester Lavorre` is lost when flattening the `appointments` array. The only information Meilisearch retains is that the patient had appointments in 2022 and 2019, and that they saw doctors `Jester Lavorre` and `Dorian Storm`:
...
This may lead to unexpected behavior during search. The following dataset shows the appointments for two patients:
...
Unintuitively, the following query returns documents `0` and `1`:
...
This happens because, when flattened, both documents contain both `2022-01-01` and `Jester Lavorre`:
...
[
  {
    "id": 0,
    "patient_name": "Imogen Temult",
    "appointments.date": ["2022-01-01"],
    "appointments.doctor": ["Jester Lavorre"]
  },
  {
    "id": 1,
    "patient_name": "Caleb Widowgast",
    "appointments.date": ["2022-01-01", "2023-01-01"],
