...
Prefix search is when document sorting starts by comparing the search query against the beginning of each word in your dataset. All documents with words that match the query term are added to the [bucket sort](https://en.wikipedia.org/wiki/Bucket_sort), before the [ranking rules](/learn/core_concepts/relevancy#ranking-rules) are applied sequentially.
...
Meilisearch also handles typos while performing the prefix search. You can [read more about the typo rules on the dedicated page](/learn/index_settings/typo_tolerance).

<Capsule intent="tip">
We also [apply splitting and concatenating on search queries](/learn/inner_workings/concat).
</Capsule>
