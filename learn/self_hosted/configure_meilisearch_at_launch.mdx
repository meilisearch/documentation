...
Migrates the database to a new Meilisearch version after updating the binary.
...
The dumpless upgrade is not currently an atomic operation. It is possible some processes fail during the upgrade, but Meilisearch still finalizes the upgrade. This may result in a corrupted database and data loss.
</Capsule>
