...
This documentation website is hosted in a [public GitHub repository](https://github.com/meilisearch/documentation). It is built with [Next.js](https://nextjs.com), written in [MDX](https://mdxjs.com), and deployed on [Vercel](https://www.vercel.com).
...
The Meilisearch documentation repository only stores the content of the documentation website. Because the code that makes up the website lives in another repository, **it is not possible to run a local copy of the documentation**.

### Handling images and other static assets

When contributing content to the Meilisearch documentation, store screenshots, images, GIFs, and videos in a relevant directory under `/assets`. 
