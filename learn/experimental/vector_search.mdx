...
Semantic search results can sometimes have very close ranking scores. This may result in hybrid searches returning fewer keyword search results than expected. Use `distributionShift` when configuring an embedder to correct result distribution with an affine transformation:
...
`distributionShift` is an optional field compatible with all embedder sources. It must be an object with two fields:

- `mean`: a number between `0` and `1` indicating the new semantic score mean  
- `sigma`: a number between `0` and `1` indicating how much semantic score values may deviate from the mean. Use values smaller than 0.4 for larger variance, and larger than 0.4 to keep results close to the mean
