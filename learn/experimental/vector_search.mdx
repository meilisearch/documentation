...
`dimensions` is an optional field. It must be bigger than zero and lower or equal to the chosen model's dimensions. This option is not compatible with `openai-text-embedding-ada-002`. Fewer dimensions may improve performance, but lead to lower search result accuracy.
