...
Semantic search results can sometimes have very close ranking scores. This may result in hybrid searches returning fewer keyword search results than expected. Use `distribution` when configuring an embedder to correct result distribution with an affine transformation:
