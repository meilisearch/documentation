...
- A [Meilisearch >= 1.6 project](/getting_started/overview)
...
Your Meilisearch instance will now contain your documents. Meilisearch runs tasks like document import asynchronously, so you might need to wait a bit for documents to be available. Consult [the asynchronous operations explanation](/products/platform/tasks) for more information on how tasks work.
...
Finally, should you want to use Meilisearch's vector search capabilities without LangChain or its hybrid search feature, refer to the [dedicated tutorial](/products/hybrid_search/getting_started).
