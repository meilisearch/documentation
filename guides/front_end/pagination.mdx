...
<Note>
When using `hitsPerPage` and `page` together with `facets`, the `facetDistribution` counts reflect the entire result set matching your query, not just the hits on the current page. This means facet counts remain consistent as users navigate between pages.
</Note>
