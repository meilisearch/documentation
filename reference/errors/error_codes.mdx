...
- The [`filter`](/reference/api/documents/list-documents-with-get) parameter is invalid
  - It should be a string, array of strings, or array of array of strings for the [get documents with POST endpoint](/reference/api/documents/documents-by-query-post)
...
- The syntax for the [`sort`](/reference/api/documents/documents-by-query-post) parameter is invalid
...
The specified value for [`searchCutoffMs](/reference/api/settings/update-search-cutoff) is invalid. It should be an integer indicating the cutoff in milliseconds.
