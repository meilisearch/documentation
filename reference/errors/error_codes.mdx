...
A document exceeds the [maximum limit of 65,535 fields](resources/known_limitations#maximum-number-of-attributes-per-document).
...
You have tried using an [experimental feature](resources/experimental_features_overview) without activating it.
...
An index with this [`uid`](/reference/api/indexes#index-object) already exists, check out our guide on [index creation](getting_started/indexes).

## `index_creation_failed`

An error occurred while trying to create an index, check out our guide on [index creation](getting_started/indexes).

## `index_not_found`

An index with this `uid` was not found, check out our guide on [index creation](getting_started/indexes).

## `index_primary_key_already_exists`

The requested index already has a primary key that [cannot be changed](getting_started/primary_key#changing-your-primary-key-with-the-update-index-endpoint).

## `index_primary_key_multiple_candidates_found`

[Primary key inference](getting_started/primary_key#meilisearch-guesses-your-primary-key) failed because the received documents contain multiple fields ending with `id`. Use the [update index endpoint](/reference/api/indexes#update-an-index) to manually set a primary key.
...
The provided [document identifier](getting_started/primary_key#document-id) does not meet the format requirements. A document identifier must be of type integer or string, composed only of alphanumeric characters (a-z A-Z 0-9), hyphens (-), and underscores (_).
...
The provided `_geo` field of one or more documents is invalid. Meilisearch expects `_geo` to be an object with two fields, `lat` and `lng`, each containing geographic coordinates expressed as a string or floating point number. Read more about `_geo` and how to troubleshoot it in [our dedicated guide](/products/geo_search/geosearch).
...
There is an error in the provided index format, check out our guide on [index creation](getting_started/indexes).
...
The value provided for the [`sortFacetValuesBy`](/reference/api/settings/faceting#faceting-object) object is incorrect. The accepted values are `alpha` or `count`.

## `invalid_settings_faceting_max_values_per_facet`

The value for the [`maxValuesPerFacet`](/reference/api/settings/faceting#faceting-object) field is invalid. It should either be an integer or set to `null`.
...
The value for the [`maxTotalHits`](/reference/api/settings/pagination#pagination-object) field is invalid. It should either be an integer or set to `null`.
...
- The [`enabled`](/reference/api/settings/typo-tolerance#typo-tolerance-object) field is invalid. It should either be a boolean or set to `null`
- The [`disableOnAttributes`](/reference/api/settings/typo-tolerance#typo-tolerance-object) field is invalid. It should either be an array of strings or set to `null`
- The [`disableOnWords`](/reference/api/settings/typo-tolerance#typo-tolerance-object) field is invalid. It should either be an array of strings or set to `null`
- The [`minWordSizeForTypos`](/reference/api/settings/typo-tolerance#typo-tolerance-object) field is invalid. It should either be an integer or set to `null`
- The value of either [`oneTypo`](/reference/api/settings/typo-tolerance#typo-tolerance-object) or [`twoTypos`](/reference/api/settings/typo-tolerance#typo-tolerance-object) is invalid. It should either be an integer or set to `null`
...
[Primary key inference](getting_started/primary_key#meilisearch-guesses-your-primary-key) failed as the received documents do not contain any fields ending with `id`. [Manually designate the primary key](getting_started/primary_key#setting-the-primary-key), or add some field ending with `id` to your documents.
