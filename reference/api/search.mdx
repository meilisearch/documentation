...
Excluding the `sort` and `geosort` ranking rules, if two documents have the same `_rankingScore`, they are returned in an unspecified order. For `sort` and `geosort`, the document order is determined by the value of the field they are sorted on.
...
<Capsule intent="note">
The `sort` and `geosort` ranking rules do not influence the `_rankingScore`. The `sort` rule isn’t shown in the `_rankingScoreDetails` object. If you have any custom rules, they will be displayed.
</Capsule>
...
| `attribute`            | `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied<br />`attribute_ranking_order`: Results sorted based on the attribute ranking order<br />`attribute_query_word_order`: Documents with attributes containing the query words at the beginning of the attribute will be considered more relevant than documents containing the query words at the end of the attributes                                                                                                                                                         |
| `sort`                 | `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `exactness`            | `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied<br />`matchType`: It has one of the following values:<br />`exactMatch`: The query exactly matches the entire value of an attribute<br />`matchesStart`: The query exactly matches the start of the value of an attribute <br />`noExactMatch` <br />`matchingWords`: for `matchesStart`, the number of exact words contained in an attribute. The higher the better<br />`maxMatchingWords`: for `matchesStart`, the maximum number of exact words contained in an attribute |
| `field_name:direction` | The `sort` rule isn’t shown. Instead, if you have any custom rules, they will be displayed here instead. `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied<br />`value`: value of the attribute in the matching document <br />If the field used for sorting is not displayed, it will be hidden. You will get:<br />`"<hidden-rule-5>"`: {<br />`"order": 5`,<br />`"value": "<hidden>"`<br />}                                                                                                                                |
