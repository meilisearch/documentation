...
Placeholder search is a search with an empty `q` parameter. Since there is no query term, the [built-in ranking rules](/learn/core_concepts/relevancy#ranking-rules) **do not apply.** Only [sort](/learn/core_concepts/relevancy#_5-sort) and [custom ranking rules](/learn/core_concepts/relevancy#custom-rules) are taken into account.
...
| **[`showRankingScoreDetails`](#show-ranking-score-details)** | Boolean          | `false`       | Display the ranking score details of a document     |
...
This is not necessary when using the `POST` route or one of our [SDKs](/learn/what_is_meilisearch/sdks).
</Capsule>
...
When `q` isn't specified, Meilisearch performs a **placeholder search**.  A placeholder search returns all searchable documents in an index, modified by any search parameters used and sorted by that index's [custom ranking rules](/learn/core_concepts/relevancy#custom-rules). Since there is no query term, the [built-in ranking rules](/learn/core_concepts/relevancy#ranking-rules) **do not apply.**
...
You can [paginate search results](/learn/front_end/pagination) by making queries combining both `offset` and `limit`.
...
You can use `hitsPerPage` and `page` to [paginate search results](/learn/front_end/pagination).
...
For more information, [read our guide on how to use filters and build filter expressions](/learn/fine_tuning_results/filtering).
...
If any parameters are invalid or missing, Meilisearch returns an [`invalid_search_filter`](/reference/errors/error_codes#invalid-search-filter) error.
...
<Capsule intent="warning">
`attributesToHighlight` will highlight matches within all attributes added to the `attributesToHighlight` array, even if those attributes are not set as [`searchableAttributes`](/learn/configuration/displayed_searchable_attributes#searchable-fields).
</Capsule>
...
[Read more about sorting search results in our dedicated guide.](/learn/advanced/sorting)
...
[You can read more about location-based sorting in our dedicated guide.](/learn/fine_tuning_results/geosearch#sorting-results-with-geopoint)
...
Adds a global ranking score field, `_rankingScore`, to each document. The `_rankingScore` is  between `0` and `1`. The higher the `_rankingScore`, the higher the document will rank.

The `sort` and `geosort` ranking rules do not influence the `_rankingScore`. Instead, the document order is determined by the value of the field they are sorted on. Excluding the `sort` and `geosort` ranking rules, if two documents have the same `_rankingScore`, they are returned in an unspecified order.

#### Example

The code sample below returns the `_rankingScore` when searching for `dragon` in `movies`:

<CodeSamples id="search_parameter_guide_show_ranking_score_1" />
  ...
  "hits": [
    {
      ...
      "poster": "https://image.tmdb.org/t/p/w500/h4m1hIVO3OKaMqEzNDEBKKOuXiQ.jpg",
      "release_date": 1165881600,
      "_rankingScore": 0.9166666666666666
    },
    {
      "id": 70057,
      "title": "Dragon",
      ...
      "poster": "https://image.tmdb.org/t/p/w500/gGHCYaUzQmmKUqxMMD5rKAyJ21a.jpg",
      "release_date": 1309737600,
      "_rankingScore": 0.9166666666666666
    },
    …
  ],
...
Adds a `_rankingScoreDetails` object to each document. This object shows details for each ranking rule.

<Capsule intent="note">
The `sort` and `geosort` ranking rules do not influence the `_rankingScore`. The `sort` rule isn’t shown in the `_rankingScoreDetails` object. If you have any custom rules, they will be displayed instead.
</Capsule>

| Ranking rule           | Field description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| :--------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `words`                | `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied<br />`matchingWords`: Number of words in the query that match in the document. The higher the better<br />`maxMatchingWords`: Maximum number of words in the query that can match in the document for this iteration of the `words` ranking rule. Usually, the query length, but if one of the query terms is set as a [stop word](/reference/api/settings#stop-words), it won’t be counted here                                                                                                                    |
| `typo`                 | `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied<br />`typoCount`: Number of typos to correct in the query so that the document matches for this iteration of the `typo` ranking rule. If the query is `badman`, it will have `0` typos when matching the movie `Angel and the Badman`, but it will have `1` typo when matching the movie `Batman returns` <br />`maxTypoCount`: Maximum number of typos possible in a document for this iteration of the `typo` ranking rule. This is based on the [typo tolerance settings](/learn/configuration/typo_tolerance) of the index                                                   |
| `proximity`            | `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `attribute`            | `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied<br />`attribute_ranking_order`: Results sorted based on the [attribute ranking order](/learn/core_concepts/relevancy#attribute-ranking-order)<br />`attribute_query_word_order`: Documents with attributes containing the query words at the beginning of the attribute will be considered more relevant than documents containing the query words at the end of the attributes                                                                                                                                                         |
| `exactness`            | `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied<br />`matchType`: It has one of the following values:<br />`exactMatch`: The query exactly matches the entire value of an attribute<br />`matchesStart`: The query exactly matches the start of the value of an attribute <br />`noExactMatch`: The query doesn't exactly match a document <br />`matchingWords`: for `matchesStart`, the number of exact words contained in an attribute. The higher the better<br />`maxMatchingWords`: for `matchesStart`, the maximum number of exact words contained in an attribute |
| `field_name:direction` | The `sort` rule isn’t displayed in the `_rankingScoreDetails` object. If you have any custom rules, they will be displayed here. `score`: The score local to this ranking rule. The higher the better<br />`order`: The order in which the ranking rules were applied<br />`value`: value of the attribute in the matching document <br />If the field used for sorting is not added to [`displayedAttributes`](/learn/configuration/displayed_searchable_attributes#displayed-fields), it will be hidden:<br />`"<hidden-rule-5>"`: {<br />`"order": 5`,<br />`"value": "<hidden>"`<br />}                                                                                                                                |

<CodeSamples id="search_parameter_guide_show_ranking_score_details_1" />
...
```json
{
  "hits": [
    ...
    …
    {
      "id": 16407,
      "title": "Dragon Lord",
      "overview": "Dragon and his madcap pal Cowboy spend their days getting into mischief…",
      "genres": [
        "Action",
        "Comedy"
      ],
