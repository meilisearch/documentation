...
The `sort` ranking rule does not influence the `_rankingScore`. Instead, the document order is determined by the value of the field they are sorted on. Excluding `sort`, if two documents have the same `_rankingScore`, they are returned in an unspecified order.
