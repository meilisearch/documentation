...
[Read more about sorting search results in our dedicated guide.](/learn/fine_tuning_results/sorting)
...
[You can read more about location-based sorting in our dedicated guide.](/learn/fine_tuning_results/geosearch#sorting-results-with-_geopoint)
...
Adds a detailed global ranking score field, `_rankingScoreDetails`, to each document. `_rankingScoreDetails` is a nested object corresponding to each active ranking rule.
...
- `score`: ranking score for this specific rule
...
- `matchType`: either `exactMatch`, `matchesStart`, or `noExactMatch`:
  - `exactMatch`: document contains an attribute matching all query terms with no other words between and in the order they were given
  - `matchesStart`: document contains an attribute with all query terms in the same position as the original query
  - `noExactMatch`: document contains an attribute with one or more query terms that don't match the original query
- `matchingWords`: the number of exact words contained in an attribute when `matchType` is `noExactMatch`
- `maxMatchingWords`: the maximum number of exact words contained in an attribute when `matchType` is `noExactMatch`

##### `field_name:direction`

The `sort` ranking rule does not appear as a single field in the score details object. Instead, each sorted attribute appears as its own field, followed by its direction.
...
- `order`: order in which this specific ranking rule was applied
- `value`: value of the field used for sorting
- `distance`: same as [_geoDistance](/learn/fine_tuning_results/geosearch#finding-the-distance-between-a-document-and-a-_geopoint)
