...
<Capsule intent="note">
By default, [this endpoint returns a maximum of 1000 results](/learn/inner_workings/known_limitations#maximum-number-of-results-per-search). If you want to scrape your database, use the [get documents endpoint](/reference/api/documents#get-documents) instead.
</Capsule>
...
You can [read more about pagination in our dedicated guide](/learn/advanced/pagination).
...
This endpoint only accepts [string filter expressions](/learn/advanced/filtering#filter-expressions).
</Capsule>

This endpoint should only be used when no API key is required. If an API key is required, use the [POST](/reference/api/search#search-in-an-index-with-post-route) route instead.
...
<Capsule intent="warning">
If [using the `GET` route to perform a search](/reference/api/search#search-in-an-index-with-get-route), all parameters must be **URL-encoded**.
...
<Capsule intent="warning">
Meilisearch only considers the first ten words of any given search query. This is necessary in order to deliver a [fast search-as-you-type experience](/learn/inner_workings/known_limitations#maximum-number-of-query-words).
</Capsule>
...
<Capsule intent="tip">
Placeholder search is particularly useful when building a [faceted search interfaces](/learn/advanced/faceted_search), as it allows users to view the catalog and alter sorting rules without entering a query.
</Capsule>
...
Phrase searches are case-insensitive and ignore [soft separators such as `-`, `,`, and `:`](/learn/inner_workings/datatypes). Using a hard separator within a phrase search effectively splits it into multiple separate phrase searches: `"Octavia.Butler"` will return the same results as `"Octavia" "Butler"`.
...
You can [paginate search results](/learn/advanced/pagination) by making queries combining both `offset` and `limit`.
...
You can use `hitsPerPage` and `page` to [paginate search results](/learn/advanced/pagination).
...
For more information on how to use filters and build filter expressions, [read our guide on filtering and filter expressions](/learn/advanced/filtering).
...
[Learn more about facet distribution in the faceted search guide.](/learn/advanced/faceted_search#configuring-and-using-facets)
...
<Capsule intent="warning">
`attributesToHighlight` will highlight matches within all attributes added to the `attributesToHighlight` array, even if those attributes are not set as [`searchableAttributes`](/learn/index_settings/displayed_searchable_attributes#searchable-fields).
</Capsule>
...
<Capsule intent="warning">
`showMatchesPosition` returns the location of matched query terms within all attributes, even attributes that are not set as [`searchableAttributes`](/learn/index_settings/displayed_searchable_attributes#searchable-fields).
</Capsule>
...
[You can read more about location-based sorting in our dedicated guide.](/learn/advanced/geosearch#sorting-results-with-geopoint)
