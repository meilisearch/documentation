...
| --- | --- |
| words | score <br />order: the ranking rule order. From 0-? <br /> matchingWords: number of words in the query that match in the document. The higher the better.<br />maxMatchingWords: max number of words in the query that can match in the document for this iteration of the words ranking rule. Usually, the query length. If one of the query terms is set as a stop word, it won’t be counted here |
| typo | score <br />order<br />typoCount: number of typos to correct in the query so that the document matches for this iteration of the typo ranking rule. If the query is badman, it will have 0 typo when matching the movie Angel and the Badman but it will have 1 typo when matching the movie Batman returns <br />maxTypoCount: max number of typos possible in a document for this iteration of the typo ranking rule. |
| proximity | score |
| attribute | score<br />- attribute_ranking_order: results sorted based on the attribute ranking order<br />- attribute_query_word_order: documents with attributes containing the query words at the beginning of the attribute will be considered more relevant than documents containing the query words at the end of the attributes. |
| sort | score |
| exactness | score <br /> matchType: has one of the following values:<br />   - exactMatch: the query exactly matches the entire value of an attribute
   <br />- matchesStart: the query exactly matches the start of the value of an attribute <br />   - noExactMatch <br />
- matchingWords: for matchesStart, the number of exact words contained in an attribute. The higher the better. <br />- maxMatchingWords: for matchesStart, the maximum number of exact words contained in an attribute <br />- score : score local to this ranking rule. The higher the better. |
| field_name:direction | The sort rule isn’t shown. Instead, if you have any custom rules, they will be displayed instead.
score<br />value: value of the attribute in the matching document <br />If the field used for sorting is not displayed, it will be hidden. You will get:
<br /><redacted>: <redacted> ”order”: 1, ”value”: <redacted> |
