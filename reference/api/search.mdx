...
| **[`retrieveVectors`](#display-vectors-in-response-experimental)** | Boolean          | `false`       | Return document vector data                         |
...
<CodeSamples id="search_parameter_guide_matching_strategy_3" />
...
<CodeSamples id="search_parameter_reference_ranking_score_threshold_1" />
...
Return document embedding data with search results. If `true`, Meilisearch will display vector data in each [document's `_vectors` field](/reference/api/documents#vectors).
