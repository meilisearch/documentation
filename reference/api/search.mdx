...
| **[`locales`](#query-locales)** | Array of strings          | `null`       | Sets query language                         |
...
By default, Meilisearch auto-detects the languages in your dataset. Use this parameter to explicitly state the language of a query.

Setting this parameter takes precedence over locales configured in the [localized attributes index setting](/reference/api/settings#localized-attributes). For example, if `localizedAttributes` declares a field contains text in japanese, setting `locales` to `chi` forces Meilisearch to ignore the original setting and parse text as chinese.
...
<CodeSamples id="search_parameter_reference_locales_1" />
