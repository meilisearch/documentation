...
If a single response contains multiple embeddings, field containing the embedding itself must be replaced with two items, `"{{embedding}}"` and `"{{..}}"`:
...
`headers` must be a JSON object whose keys represent the name of additional headers to send in requests to embedders, and the values represent the value of these additional headers.
...
If `headers` includes either fields, the declared values take precedence over the default.
