...
| **[`searchCutoffMs`](#search-cut-off)**              | Integer          | `null`, or 1500ms                                                                                | Maximum duration of a search query                                               |
...
[To learn more about filterable attributes, refer to our dedicated guide.](/learn/fine_tuning_results/filtering)
...
[To learn more about paginating search results with Meilisearch, refer to our dedicated guide.](/learn/front_end/pagination)
...
Configure the maximum duration of a search query. Meilisearch will interrupt any search taking longer than the cut-off value.
...
Get an index's search cut-off value.
...
<CodeSamples id="get_search_cut_off__1"/>
...
### Update search cut-off

<RouteHighlighter method="PUT" route="/indexes/{index_uid}/settings/search-cut-off-ms" />

Update an index's search cut-off value.
...
A single integer indicating the cut-off value in milliseconds.

#### Example

<CodeSamples id="update_search_cut_off_1"/>
...
### Reset search cut-off

<RouteHighlighter method="DELETE" route="/indexes/{index_uid}/settings/search-cut-off-ms"/>

Reset an index's search cut-off value to its default value, `null`. This translates to a cut-off of 1500ms.
...
<CodeSamples id="reset_search_cut_off_1"/>
