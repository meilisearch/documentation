...
Calculating the [proximity ranking rule](/learn/core_concepts/relevancy#3-proximity) is a resource-intensive operation. Lowering its precision may significantly improve performance and will have little impact on result relevancy in most use-cases.

`proximityPrecision` accepts one of the following string values:

- `"byWord"`: calculate precise distance between query terms. Higher precision, but may lead to longer indexing time. This is the default setting
- `"byAttribute"`: detect if multiple query terms are present in the same attribute. Lower precision, but shorter indexing time
...
Setting `proximityPrecision` to `null` resets it to the default value: `"byWord"`.
