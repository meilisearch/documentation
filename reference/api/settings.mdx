...
You may also set attributePatterns to *. In this case Meilisearch will treat all fields as filterable.
...
- `filter`: A list outlining the filter types for the specified attributes. Must be an object and accepts the following fields:
  - `equality`: Enables `=`, `!=`, `IN`, `EXISTS`, `NOT`, `AND`, and `OR`. Boolean, defaults to `true`
  - `comparison`: Enables `>`, `>=`, `<`, `<=`, `TO`, `EXISTS`, `NOT`, `AND`, and `OR`. Boolean, defaults to `false`
...
<Capsule intent="warning" title="_geo field and filter features">
If set as a filterable attribute, `_geo` automatically activates all search features. Meilisearch will throw an error if you try to disable any `features` for a `_geo` field.
</Capsule>
