...
| **[`prefixSearch`](#prefix-search)**     | String           | `"indexingTime"`                                                                                       | When and if Meilisearch should return results only matching the beginning of query                     |
...
<CodeSamples id="get_localized_attribute_settings_1" />
...
Reset an index's facet search settings to its default value.
...
Prefix search is the process through which Meilisearch matches documents that begin with a specific query, instead of only exact matches. This is a resource-intensive operation that happens during indexing by default.
...
This field is optional when using `ollama` and `openAi` embedders.
