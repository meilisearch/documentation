...
An alphanumeric key value generated by Meilisearch by hasing the `uid` and the master key on API key creation. Used for authorization when [making calls to a protected Meilisearch instance](/learn/security/master_api_keys.md#communicating-with-a-protected-instance).
...
::: note
Since `key` is a hash of the `uid` and master key, `key` values are deterministic between instances sharing the same configuration. This means if the master key changes, all key values are automatically changed and that API key resources are propagated into dumps and snapshots without displaying the `key` field in clear.
:::
