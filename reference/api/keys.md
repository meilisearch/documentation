...
This value is also used as the `:key` path variable to [update](#update-a-key), [delete](#delete-a-key), or [get information on](#get-one-key) a specific key.
...
Get information on the specified key. Attempting to use this endpoint with a non-existent, deleted, or changed key value will result in [an error](http://docs.meilisearch.com/errors#api_key_not_found).
...
Get information on all existing API keys. Expired keys are included in the response, but deleted keys are not.
...
Updates to keys are **partial**. This means that you should provide only the fields you intend to update, as any fields not present in the payload will remain unchanged.
