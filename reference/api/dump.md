...
**MeiliSearch only processes one dump at a time.** If you attempt to create a dump while another dump is still processing, MeiliSearch will throw a [`dump_already_processing` error](https://docs.meilisearch.com/errors/#dump_already_processing). While a dump is processing, the **task queue is paused and no write operations can occur on the database.**
