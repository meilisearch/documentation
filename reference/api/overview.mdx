...
```
 cat ~/movies.json | gzip | curl -X POST 'https://MEILISEARCH_URL/indexes/movies/documents' --data-binary @- -H 'Content-Type: application/json' -H 'Content-Encoding: gzip'
```
...
```
curl -sH 'Accept-encoding: gzip' 'https://MEILISEARCH_URL/indexes/movies/search' | gzip -
```
