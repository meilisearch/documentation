...
```
 cat ~/movies.json | gzip | curl -X POST 'http://MEILISEARCH_URL/indexes/movies/documents' --data-binary @- -H 'Content-Type: application/json' -H 'Content-Encoding: gzip'
```
...
```
curl -sH 'Accept-encoding: gzip' 'http://MEILISEARCH_URL/indexes/movies/search' | gzip -
```
