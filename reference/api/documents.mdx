...
`_vector` is a special document attribute containing vector embeddings for AI-powered search. Its may be array of numbers:
...
`regenerate` must be a Boolean value. If `regenerate` is `true`, the supplied embeddings will be overwritten the next time Meilisearch automatically generates document embeddings after a document update. If `regenerate` is `false`, the document's embeddings will be automatically replaced by an embedder.
