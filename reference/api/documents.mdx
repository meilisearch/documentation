...
| Query Parameter    | Default Value | Description                                                                                                                             |
| :----------------- | :------------ | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **`primaryKey`**   | `null`        | [Primary key](/learn/core_concepts/primary_key#primary-field) of the index                                                              |
...
`_vectors` is a special document attribute containing vector embeddings for AI-powered search. It must be an object.

Each key of the `_vectors` object must be the name of a configured embedder and correspond to an array of numbers:
...
Keys of the `_vectors` object may also correspond to an object with two fields, `embeddings` and `regenerate`:
...
`embeddings` must be an array of numbers representing a single embedding for that document. It may also be an array of arrays of numbers representing multiple embeddings for that document. `embeddings` is an optional field.

`regenerate` must be a Boolean value. If `regenerate` is `true`, Meilisearch automatically generates embeddings for that document immediately and every time the document is updated. If `regenerate` is `false`, Meilisearch keeps the last value of the embeddings on document updates.
...
<Capsule intent="note">
If the key for an embedder inside `_vectors` is empty or `null`, Meilisearch treats the document as not having any embeddings for that embedder. This document is then ignored during AI-powered searches.
</Capsule>
...
| Query Parameter    | Default Value | Description                                                                                                                             |
| :----------------- | :------------ | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **`primaryKey`**   | `null`        | [Primary key](/learn/core_concepts/primary_key#primary-field) of the documents                                                          |
| **`csvDelimiter`** | `","`         | Configure the character separating CSV fields. Must be a string containing [one ASCII character](https://www.rfc-editor.org/rfc/rfc20). |
