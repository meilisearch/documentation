...
`function` must be a string with a function Meilisearch will apply to all selected documents. By default this function has access to a single variable, `doc`, representing the document you are currently editing. This is a required field.

#### `filter`

`filter` must be a string containing a filter expression. Use when you want only to apply `function` to a subset of all documents.

#### `context`

Use `context` to make data available inside the `function` scope. By default a function only has access to the document it is editing.
