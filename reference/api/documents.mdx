...
<Capsule intent="note">
This endpoint is only available in v1.2 and above.
</Capsule>
...
`filter` will not work without first explicitly adding attributes to the [`filterableAttributes` list](/reference/api/settingss#update-filterable-attributes). [Learn more about filters in our dedicated guide.](/learn/advanced/filtering#filter-basics)
</Capsule>
...
Using the query parameters `offset` and `limit`, you can browse through all your documents.`filter` must be a string. To create [filter expressions](/learn/advanced/filtering#filter-expressions) use `AND` or `OR`.
...
<Capsule intent="note">
`filter` is only available in v1.2 and above.
</Capsule>
