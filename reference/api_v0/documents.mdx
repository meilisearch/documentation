...
<Warning>
`filter` will not work without first explicitly adding attributes to the [`filterableAttributes` list](/reference/api/settings/get-filterableattributes#update-filterable-attributes). [Learn more about filters in our dedicated guide.](/products/faceted_search/filter_search_results)
</Warning>
...
Using the query parameters `offset` and `limit`, you can browse through all your documents.`filter` must be a string. To create [filter expressions](/products/faceted_search/filter_expression_reference) use `AND` or `OR`.
...
| :------------------ | :------------- | :------------------------------------------------------------------------------------ |
| **`index_uid`** *   | String         | [`uid`](getting_started/glossary/indexes#index-uid) of the requested index                |
| **`document_id`** * | String/Integer | [Document id](getting_started/glossary/primary_key#document-id) of the requested document |
...
If you send an already existing document (same [document id](getting_started/glossary/primary_key#document-id)) the **whole existing document** will be overwritten by the new document. Fields that are no longer present in the new document are removed. For a partial update of the document see the [add or update documents](/reference/api/documents/overview#add-or-update-documents) endpoint.
...
| Query Parameter    | Default Value | Description                                                                                                                             |
| :----------------- | :------------ | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **`primaryKey`**   | `null`        | [Primary key](getting_started/glossary/primary_key#primary-field) of the index                                                              |
...
| **`customMetadata`** | `null`         | An arbitrary string accessible via the [generated task object](/reference/api/tasks/overview#custommetadata) |
...
If you want to [set the primary key of your index on document addition](getting_started/glossary/primary_key#setting-the-primary-key-on-document-addition), it can only be done **the first time you add documents** to the index. After this, the `primaryKey` parameter will be ignored if given.
...
If you send an already existing document (same [document id](getting_started/glossary/primary_key#document-id)) the old document will be only partially updated according to the fields of the new document. Any fields not present in the new document are kept and remain unchanged.
...
To completely overwrite a document, check out the [add or replace documents route](/reference/api/documents/overview#add-or-replace-documents).

If you want to set the [**primary key** of your index](getting_started/glossary/primary_key#setting-the-primary-key-on-document-addition) through this route, you may only do so **the first time you add documents** to the index. If you try to set the primary key after having added documents to the index, the task will return an error.
...
| Query Parameter    | Default Value | Description                                                                                                                             |
| :----------------- | :------------ | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **`primaryKey`**   | `null`        | [Primary key](getting_started/glossary/primary_key#primary-field) of the documents                                                          |
| **`csvDelimiter`** | `","`         | Configure the character separating CSV fields. Must be a string containing [one ASCII character](https://www.rfc-editor.org/rfc/rfc20). |
| **`customMetadata`** | `null`         | An arbitrary string accessible via the [generated task object](/reference/api/tasks/overview#custommetadata) |
| **`skipCreation`**   | `false`       | If `true`, updates existing documents, but does not add new documents to the index |
...
This document is an update of the document found in [add or replace document](/reference/api/documents/overview#add-or-replace-documents).

The documents are matched because they have the same [primary key](/getting_started/glossary/documents#primary-field) value `id: 287947`. This route will update the `title` field as it changed from `Shazam` to `Shazam ⚡️` and add the new `genres` field to that document. The rest of the document will remain unchanged.
