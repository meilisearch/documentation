...
| :--------------- | :-------------- | :------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`uid`**        | String          | N/A           | [Unique identifier](/getting_started/glossary/indexes#index-uid) of the index. Once created, it cannot be changed                                                                                                                                                |
...
| **`primaryKey`** | String / `null` | `null`        | [Primary key](/getting_started/glossary/primary_key#primary-field) of the index. If not specified, Meilisearch [guesses your primary key](/getting_started/glossary/primary_key#meilisearch-guesses-your-primary-key) from the first document you add to the index |
...
| Name             | Type            | Default value | Description                                                                               |
| :--------------- | :-------------- | :------------ | :---------------------------------------------------------------------------------------- |
| **`uid`** *      | String          | N/A           | [`uid`](/getting_started/glossary/indexes#index-uid) of the requested index                 |
| **`primaryKey`** | String / `null` | `null`        | [`Primary key`](/getting_started/glossary/primary_key#primary-field) of the requested index |
...
You can use the response's `taskUid` to [track the status of your request](/reference/api/tasks/overview#get-one-task).

## Update an index

<RouteHighlighter method="PATCH" path="/indexes/{index_uid}" />

Update an index's [primary key](/getting_started/glossary/primary_key#primary-key). You can freely update the primary key of an index as long as it contains no documents.
...
| Name               | Type            | Default value | Description                                                                               |
| :----------------- | :-------------- | :------------ | :---------------------------------------------------------------------------------------- |
| **`primaryKey`** * | String / `null` | N/A           | [`Primary key`](/getting_started/glossary/primary_key#primary-field) of the requested index |
...
[To learn more about index swapping, refer to this short guide.](/getting_started/glossary/indexes#swapping-indexes)
...
<Note>
Since `indexSwap` is a [global task](/products/platform/asynchronous_operations#global-tasks), the `indexUid` is always `null`.
</Note>
