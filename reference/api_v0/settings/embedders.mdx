...
`searchFragments` instructs Meilisearch how to parse fields present in a query's [`media` search parameter](/reference/api/indexes/search-with-post#media). It must be an object following the same structure as the [`indexingFragments`](/reference/api/settings/get-embedders#indexingfragments) object:
...
Use Liquid templates to interpolate search query data into the fragment fields, where `{{media.*}}` gives you access to all [multimodal data received with a query](/reference/api/indexes/search-with-post#media) and `{{q}}` gives you access to the regular textual query:
