...
Chat queries only search the indexes its API key can access. The default chat API key has access to all indexes. To limit access, you must either create a new key, or [generate a tenant token](/products/platform/generate_tenant_tokens) from the default chat API key.
