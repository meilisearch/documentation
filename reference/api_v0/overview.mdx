...
If you are new to Meilisearch, check out the [getting started](/resources/self_hosting/getting_started).
...
Since the `/tasks` endpoint uses a different type of pagination, the response contains different fields. You can read more about it in the [tasks API reference](/reference/api/tasks/overview#get-tasks).
...
Only the [add documents](/reference/api/documents/overview#add-or-replace-documents) and [update documents](/reference/api/documents/overview#add-or-update-documents) endpoints accept NDJSON and CSV. For all others, use `Content-Type: application/json`.
