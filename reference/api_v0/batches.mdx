...
The `/batches` route gives information about the progress of batches of [asynchronous operations](/products/platform/asynchronous_operations).
...
**Type**: Object<br />
**Description**: Basic information on the types tasks in a batch. Consult the [task object reference](/reference/api/tasks/overview#details) for an exhaustive list of possible values.
...
Object listing the [status of each task](/reference/api/tasks/overview#status) in the batch. Contains five keys whose values correspond to the number of tasks with that status.
...
Batch results are [paginated](/products/platform/paginating_tasks) and can be [filtered](/products/platform/filtering_tasks) with query parameters.
