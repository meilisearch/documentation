...
<Note>
By default, [this endpoint returns a maximum of 1000 results](/resources/known_limitations#maximum-number-of-results-per-search). If you want to scrape your database, use the [get documents endpoint](/reference/api/documents#get-documents-with-post) instead.
</Note>
...
<Warning>
Meilisearch only considers the first ten words of any given search query. This is necessary in order to deliver a [fast search-as-you-type experience](/resources/known_limitations#maximum-number-of-query-words).
</Warning>
...
<Warning>
Setting `offset` to a value greater than an [index's `maxTotalHits`](/reference/api/settings/pagination#update-pagination-settings) returns an empty array.
</Warning>
...
<Warning>
A search query cannot return more results than configured in [`maxTotalHits`](/reference/api/settings/pagination#pagination-object), even if the value of `limit` is greater than the value of `maxTotalHits`.
</Warning>
...
[You can read more about location-based sorting in the dedicated guide.](/products/geo_search/geosearch#sorting-results-with-_geopoint)
...
- `order`: order in which this ranking rule was applied
- `value`: value of the field used for sorting
- `distance`: same as [_geoDistance](/products/geo_search/geosearch#finding-the-distance-between-a-document-and-a-_geopoint)
...
**Expected value**: array of [supported ISO-639 locales](/reference/api/settings/localized-attributes#localized-attributes-object)<br />
**Default value**: `[]`
...
`media` must be an object whose fields must correspond to the data required by one [search fragment](/reference/api/settings/embedders#searchfragments). `media` must match a single search fragment. If `media` matches more than one fragment or no search fragments at all, Meilisearch will return an error.
