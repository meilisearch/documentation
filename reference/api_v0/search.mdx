...
<Note>
By default, [this endpoint returns a maximum of 1000 results](/resources/help/known_limitations#maximum-number-of-results-per-search). If you want to scrape your database, use the [get documents endpoint](/reference/api/documents/overview#get-documents-with-post) instead.
</Note>
...
Neither `estimatedTotalHits` nor `totalHits` can exceed the limit configured in [the `maxTotalHits` index setting](/reference/api/settings/get-pagination).

You can [read more about pagination in our dedicated guide](/guides/performance/pagination).

### Example

<CodeSamplesSearchPost1 />
...
This endpoint should only be used when no API key is required. If an API key is required, use the [POST](/reference/api/indexes/search-with-post#search-in-an-index-with-post) route instead.
...
<Warning>
If [using the `GET` route to perform a search](/reference/api/indexes/search-with-post#search-in-an-index-with-get), all parameters must be **URL-encoded**.

This is not necessary when using the `POST` route or one of our [SDKs](/resources/help/sdks).
</Warning>
...
<Warning>
Meilisearch only considers the first ten words of any given search query. This is necessary in order to deliver a [fast search-as-you-type experience](/resources/help/known_limitations#maximum-number-of-query-words).
</Warning>
...
<CodeSamplesSearchParameterGuideQuery1 />
...
When `q` isn't specified, Meilisearch performs a **placeholder search**.  A placeholder search returns all searchable documents in an index, modified by any search parameters used and sorted by that index's [custom ranking rules](/products/full_text_search/custom_ranking_rules). Since there is no query term, the [built-in ranking rules](/products/full_text_search/ranking_rules) **do not apply.**
...
<Tip>
Placeholder search is particularly useful when building a [faceted search interfaces](/products/faceted_search/search_with_facet_filters), as it allows users to view the catalog and alter sorting rules without entering a query.
</Tip>
...
Phrase searches are case-insensitive and ignore [soft separators such as `-`, `,`, and `:`](/resources/internals/datatypes). Using a hard separator within a phrase search effectively splits it into multiple separate phrase searches: `"Octavia.Butler"` will return the same results as `"Octavia" "Butler"`.

You can combine phrase search and normal queries in a single search request. In this case, Meilisearch will first fetch all documents with exact matches to the given phrase(s), and [then proceed with its default behavior](/products/full_text_search/ranking_rules).
...
You can [paginate search results](/guides/performance/pagination) by making queries combining both `offset` and `limit`.

<Warning>
Setting `offset` to a value greater than an [index's `maxTotalHits`](/reference/api/settings/get-pagination#update-pagination-settings) returns an empty array.
</Warning>
...
<CodeSamplesSearchParameterGuideOffset1 />
...
<Warning>
A search query cannot return more results than configured in [`maxTotalHits`](/reference/api/settings/get-pagination#pagination-object), even if the value of `limit` is greater than the value of `maxTotalHits`.
</Warning>
...
<CodeSamplesSearchParameterGuideLimit1 />
...
You can use `hitsPerPage` and `page` to [paginate search results](/guides/performance/pagination).
...
<Warning>
`hitsPerPage` and `page` are resource-intensive options and might negatively impact search performance. This is particularly likely if [`maxTotalHits`](/reference/api/settings/get-pagination) is set to a value higher than its default.
</Warning>
...
<CodeSamplesSearchParameterGuideHitsperpage1 />
...
<CodeSamplesSearchParameterGuidePage1 />
...
Uses filter expressions to refine search results. Attributes used as filter criteria must be added to the [`filterableAttributes` list](/reference/api/settings/get-filterableattributes).

For more information, [read our guide on how to use filters and build filter expressions](/products/faceted_search/filter_search_results).
...
By default, `facets` returns a maximum of 100 facet values for each faceted field. You can change this value using the `maxValuesPerFacet` property of the [`faceting` index settings](/reference/api/settings/get-faceting).
...
<CodeSamplesSearchParameterGuideFacetStats1 />
...
[Learn more about facet distribution in the faceted search guide.](/products/faceted_search/search_with_facet_filters)
...
This behavior is similar to the [`distinctAttribute` index setting](/reference/api/settings/get-distinctattribute), but can be configured at search time. `distinctAttribute` acts as a default distinct attribute value you may override with `distinct`.

#### Examples

<CodeSamplesSearchParameterReferenceDistinct1 />
...
If no value is specified, `attributesToRetrieve` uses the [`displayedAttributes` list](/reference/api/settings/get-displayedattributes), which by default contains all attributes found in the documents.
...
<CodeSamplesSearchParameterGuideRetrieve1 />
...
<CodeSamplesSearchParameterGuideCrop1 />
...
<CodeSamplesSearchParameterGuideCropMarker1 />
...
<Note>
`attributesToHighlight` also highlights terms configured as [synonyms](/reference/api/settings/get-synonyms) and [stop words](/reference/api/settings/get-stopwords).
</Note>

<Warning>
`attributesToHighlight` will highlight matches within all attributes added to the `attributesToHighlight` array, even if those attributes are not set as [`searchableAttributes`](/products/full_text_search/displayed_searchable_attributes#searchable-fields).
</Warning>
...
<CodeSamplesSearchParameterGuideHighlight1 />
...
<CodeSamplesSearchParameterGuideHighlightTag1 />
...
<Warning>
`showMatchesPosition` returns the location of matched query terms within all attributes, even attributes that are not set as [`searchableAttributes`](/products/full_text_search/displayed_searchable_attributes#searchable-fields).
</Warning>
...
<CodeSamplesSearchParameterGuideShowMatchesPosition1 />
...
[Read more about sorting search results in our dedicated guide.](/products/faceted_search/sort_search_results)

#### Example

You can search for science fiction books ordered from cheapest to most expensive:

<CodeSamplesSearchParameterGuideSort1 />
...
[You can read more about location-based sorting in the dedicated guide.](/products/geo_search/overview#sorting-results-with-_geopoint)
...
<CodeSamplesSearchParameterGuideMatchingStrategy1 />
...
<CodeSamplesSearchParameterGuideMatchingStrategy2 />
...
<CodeSamplesSearchParameterGuideMatchingStrategy3 />
...
<CodeSamplesSearchParameterGuideShowRankingScore1 />
...
- `order`: order in which this ranking rule was applied
- `value`: value of the field used for sorting
- `distance`: same as [_geoDistance](/products/geo_search/overview#finding-the-distance-between-a-document-and-a-_geopoint)
...
<CodeSamplesSearchParameterReferenceRankingScoreThreshold1 />
...
<CodeSamplesSearchParameterGuideAttributesToSearchOn1 />
...
Return document and query embeddings with search results. If `true`, Meilisearch will display vector data in each [document's `_vectors` field](/reference/api/documents/overview#_vectors).

<Warning>
`_vectors` must be included in the [displayedAttributes](/reference/api/settings/overview#displayed-attributes) list to be returned in the response.
</Warning>
...
**Expected value**: array of [supported ISO-639 locales](/reference/api/settings/get-localizedattributes#localized-attributes-object)<br />
**Default value**: `[]`
...
In case of a mismatch between `locales` and the [localized attributes index setting](/reference/api/settings/get-localizedattributes), this parameter takes precedence.

<Note>
`locales` and [`localizedAttributes`](/reference/api/settings/get-localizedattributes) have the same goal: explicitly state the language used in a search when Meilisearch's language auto-detection is not working as expected.
...
`media` must be an object whose fields must correspond to the data required by one [search fragment](/reference/api/settings/get-embedders#searchfragments). `media` must match a single search fragment. If `media` matches more than one fragment or no search fragments at all, Meilisearch will return an error.
...
<Note>
This is an experimental feature. Contact Meilisearch Cloud support to enable it for your projects.
</Note>

Adds user context to [personalize search results according to user profile](/products/personalization/making_personalized_search_queries).
