...
The `/tasks` route gives information about the progress of [asynchronous operations](/products/platform/tasks).
...
<Note>
This value is always `null` for [global tasks](/products/platform/tasks#global-tasks).
</Note>
...
| Name          | Description                                     |
| :--------------- | :--------------------------------------------|
| **`remoteName`** | The name of the [remote](/reference/api/network/overview) |
| **`taskUid`**    | The uid of the task of origin                |

`remotes` is itself an object whose keys are the [remotes](/reference/api/network/overview) and values an object with a single key that is either `task_uid` or `error`:
...
| **`indexUid`**   | String  | Unique index identifier (always `null` for [global tasks](/products/platform/tasks#global-tasks))              |
...
Task results are [paginated](/products/platform/tasks#paginating-tasks) and can be [filtered](/products/platform/tasks#filtering-tasks).

### Query parameters

| Query Parameter        | Default Value                  | Description                                                                                                                                                        |
| :--------------------- | :----------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`uids`**             | `*` (all uids)                 | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `uid`. Separate multiple task `uids` with a comma (`,`)                           |
| **`batchUids`**             | `*` (all batch uids)                 | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `batchUid`. Separate multiple `batchUids` with a comma (`,`)                           |
| **`statuses`**         | `*` (all statuses)             | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `status`. Separate multiple task `statuses` with a comma (`,`)                    |
| **`types`**            | `*` (all types)                | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `type`. Separate multiple task `types` with a comma (`,`)                         |
| **`indexUids`**        | `*` (all indexes)              | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `indexUid`. Separate multiple task `indexUids` with a comma (`,`). Case-sensitive |
...
| **`canceledBy`**       | N/A                            | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `canceledBy` field. Separate multiple task `uids` with a comma (`,`)         |
| **`beforeEnqueuedAt`** | `*` (all tasks)                | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `enqueuedAt` field                                                                 |
| **`beforeStartedAt`**  | `*` (all tasks)                | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `startedAt` field                                                                  |
| **`beforeFinishedAt`** | `*` (all tasks)                | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `finishedAt` field                                                                 |
| **`afterEnqueuedAt`**  | `*` (all tasks)                | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `enqueuedAt` field                                                                 |
| **`afterStartedAt`**   | `*` (all tasks)                | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `startedAt` field                                                                  |
| **`afterFinishedAt`**  | `*` (all tasks)                | [Filter tasks](/products/platform/tasks#filtering-tasks) by their `finishedAt` field                                                                 |
...
[To learn more about filtering tasks, refer to our dedicated guide.](/products/platform/tasks#filtering-tasks)
...
<Note>
Since `taskCancelation` is a [global task](/products/platform/tasks#global-tasks), its `indexUid` is always `null`.
</Note>
...
The API key used must have access to all indexes (`"indexes": [*]`) and the [`task.cancel`](/reference/api/keys/overview#actions) action.
...
<Note>
Since `taskDeletion` is a [global task](/products/platform/tasks#global-tasks), its `indexUid` is always `null`.
</Note>
...
The API key used must have access to all indexes (`"indexes": [*]`) and the [`task.delete`](/reference/api/keys/overview#actions) action.
