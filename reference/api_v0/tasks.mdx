...
<Note>
This value is always `null` for [global tasks](/products/platform/asynchronous_operations#global-tasks).
</Note>
...
| **`indexUid`**   | String  | Unique index identifier (always `null` for [global tasks](/products/platform/asynchronous_operations#global-tasks))              |
...
Task results are [paginated](/products/platform/paginating_tasks) and can be [filtered](/products/platform/filtering_tasks).

### Query parameters

| Query Parameter        | Default Value                  | Description                                                                                                                                                        |
| :--------------------- | :----------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`uids`**             | `*` (all uids)                 | [Filter tasks](/products/platform/filtering_tasks) by their `uid`. Separate multiple task `uids` with a comma (`,`)                           |
| **`batchUids`**             | `*` (all batch uids)                 | [Filter tasks](/products/platform/filtering_tasks) by their `batchUid`. Separate multiple `batchUids` with a comma (`,`)                           |
| **`statuses`**         | `*` (all statuses)             | [Filter tasks](/products/platform/filtering_tasks) by their `status`. Separate multiple task `statuses` with a comma (`,`)                    |
| **`types`**            | `*` (all types)                | [Filter tasks](/products/platform/filtering_tasks) by their `type`. Separate multiple task `types` with a comma (`,`)                         |
| **`indexUids`**        | `*` (all indexes)              | [Filter tasks](/products/platform/filtering_tasks) by their `indexUid`. Separate multiple task `indexUids` with a comma (`,`). Case-sensitive |
...
| **`canceledBy`**       | N/A                            | [Filter tasks](/products/platform/filtering_tasks) by their `canceledBy` field. Separate multiple task `uids` with a comma (`,`)         |
| **`beforeEnqueuedAt`** | `*` (all tasks)                | [Filter tasks](/products/platform/filtering_tasks) by their `enqueuedAt` field                                                                 |
| **`beforeStartedAt`**  | `*` (all tasks)                | [Filter tasks](/products/platform/filtering_tasks) by their `startedAt` field                                                                  |
| **`beforeFinishedAt`** | `*` (all tasks)                | [Filter tasks](/products/platform/filtering_tasks) by their `finishedAt` field                                                                 |
| **`afterEnqueuedAt`**  | `*` (all tasks)                | [Filter tasks](/products/platform/filtering_tasks) by their `enqueuedAt` field                                                                 |
| **`afterStartedAt`**   | `*` (all tasks)                | [Filter tasks](/products/platform/filtering_tasks) by their `startedAt` field                                                                  |
| **`afterFinishedAt`**  | `*` (all tasks)                | [Filter tasks](/products/platform/filtering_tasks) by their `finishedAt` field                                                                 |
...
[To learn more about filtering tasks, refer to our dedicated guide.](/products/platform/filtering_tasks)
...
<Note>
Since `taskCancelation` is a [global task](/products/platform/asynchronous_operations#global-tasks), its `indexUid` is always `null`.
</Note>
...
<Note>
Since `taskDeletion` is a [global task](/products/platform/asynchronous_operations#global-tasks), its `indexUid` is always `null`.
</Note>
