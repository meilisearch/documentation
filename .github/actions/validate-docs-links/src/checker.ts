...
async function getAllMdxFilePaths(basePath: string): Promise<RouteFragment[]> {
  const sidebarLearn: ConfigSchema = JSON.parse(
    await fs.readFile(path.join(basePath, "config/sidebar-learn.json"), "utf8")
  );
  const sidebarReference: ConfigSchema = JSON.parse(
    await fs.readFile(
      path.join(basePath, "config/sidebar-reference.json"),
      "utf8"
    )
  );
  const sidebarGuides: ConfigSchema = JSON.parse(
    await fs.readFile(path.join(basePath, "config/sidebar-guides.json"), "utf8")
  );
  const footer: FooterConfigSchema = JSON.parse(
    await fs.readFile(path.join(basePath, "config/sidebar-footer.json"), "utf8")
  );

  const config = [
    ...sidebarLearn.map((group) => ({
      ...group,
      slug: path.join("learn", group.slug),
    })),
    ...sidebarReference.map((group) => ({
      ...group,
      slug: path.join("reference/", group.slug),
    })),
    ...sidebarGuides.map((group) => ({
      ...group,
      slug: path.join("guides/", group.slug),
    })),
  ];

  let allRoutes: RouteSchema[] = [
    { source: path.join(basePath, "home.mdx"), slug: "", label: "Homepage" },
  ];
  ...
  footer.forEach(
    (item) =>
      "source" in item &&
      allRoutes.push({ ...item, source: path.join(basePath, item.source) })
  );
...
function traverseTreeAndValidateLinks(
  tree: any,
  doc: Document,
  setFailed: FailureFunction
): Errors {
  ...
  function traverseRecursively(tree: any) {
    try {
      visit(tree, (node) => validateNodes(node));
...
export async function validateAllInternalLinks(
  basePath: string,
  setFailed: FailureFunction,
  sha?: string
): Promise<void> {
  try {
    ...
    documentMap = new Map(
      await Promise.all(
        allMdxFilePaths.map((route) =>
          prepareDocumentMapEntry(route, setFailed)
        )
      )
    );
    ...
    const docProcessingPromises = allMdxFilePaths.map(async (route) => {
      ...
      if (doc) {
        const tree = (await markdownProcessor.process(doc.body)).result;
...
if (process.argv[2] === "--run-local-checker") {
  validateAllInternalLinks("../../../", (message) => {
