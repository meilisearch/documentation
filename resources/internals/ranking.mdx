...
Most search engines rank results using a single relevancy score computed from a formula like BM25. Meilisearch takes a fundamentally different approach: it evaluates multiple ranking criteria sequentially, giving you transparent control over what matters most and delivering results that feel right to end users.

## How traditional search ranking works

### BM25: the industry default

[BM25](https://en.wikipedia.org/wiki/Okapi_BM25) (Best Matching 25) is the ranking function used by Elasticsearch, OpenSearch, Lucene, PostgreSQL full-text search, and most traditional search engines. It computes a single numeric score per document based on:
...
The formula produces a score like `8.72` or `3.14`, and results are sorted by this number in descending order.
...
Meilisearch's ranking system works alongside [hybrid search](/learn/ai_powered_search/hybrid_search/overview). When you enable an [embedder](/learn/ai_powered_search/getting_started_with_ai_search), Meilisearch combines keyword-based ranking (the rules above) with vector similarity in a single query:
...
| | BM25 (Elasticsearch, PostgreSQL, etc.) | Meilisearch multi-criteria ranking |
| :--- | :--- | :--- |
...
| **Word proximity** | Not a scoring factor | Dedicated ranking rule |
| **Field importance** | Numeric boosts with complex interactions | Ordered list, first field always wins |
| **Prefix search** | Requires analyzer configuration | Automatic on last query word |
...
| **Semantic search** | Separate system (kNN, vector DB) | Integrated via `hybrid` parameter |
