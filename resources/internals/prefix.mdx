...
You can disable prefix search entirely using the [`prefixSearch` index setting](/reference/api/settings/get-prefix-search). Set it to `disabled` to turn off prefix search for an index. The default value is `indexingTime`, which enables prefix search.
