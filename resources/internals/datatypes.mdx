...
Distance plays an essential role in determining whether documents are relevant since [one of the ranking rules is the **proximity** rule](/products/full_text_search/ranking_rules). The proximity rule sorts the results by increasing distance between matched query terms. Then, two words separated by a soft space are closer and thus considered **more relevant** than two words separated by a hard space.
...
You can add [custom ranking rules](/products/full_text_search/custom_ranking_rules) to create an ascending or descending sorting rule on a given attribute that has a numeric value in the documents.

You can also create [filters](/products/faceted_search/filter_search_results). The `>`, `>=`, `<`, `<=`, and `TO` relational operators apply only to numerical values.
