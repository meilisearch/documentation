...
If your [`pkgVersion`](/reference/api/version/overview#version-object) is 0.21 or above, you can jump to [creating the dump](#create-the-dump). If not, proceed to the next step.
...
When creating dumps using Meilisearch versions below v0.21, all fields must be [displayed](/products/full_text_search/displayed_searchable_attributes#displayed-fields) in order to be saved in the dump.
...
Before creating your dump, make sure that your [dump directory](/resources/self_hosting/configuration#dump-directory) is somewhere accessible. By default, dumps are created in a folder called `dumps` at the root of your Meilisearch directory.
...
<Warning>
Due to an error allowing malformed `_geo` fields in Meilisearch **v0.27, v0.28, and v0.29**, you might not be able to import your dump. Please ensure the `_geo` field follows the [correct format](/products/geo_search/overview#preparing-documents-for-location-based-search) before creating your dump.
</Warning>
...
Use the `taskUid` to [track the status](/reference/api/tasks/overview#get-one-task) of your dump. Keep in mind that the process can take some time to complete.
...
Instead of deleting `data.ms`, we suggest creating a backup in case something goes wrong. `data.ms` should be at the root of the Meilisearch binary unless you chose [another location](/resources/self_hosting/configuration#database-path).
...
If required, set `displayedAttributes` back to its previous value using the [update displayed attributes endpoint](/reference/api/settings/get-displayedattributes#update-displayed-attributes).
