...
<Warning>
Due to an error allowing malformed `_geo` fields in Meilisearch **v0.27, v0.28, and v0.29**, you might not be able to import your dump. Please ensure the `_geo` field follows the [correct format](/products/geo_search/geosearch#preparing-documents-for-location-based-search) before creating your dump.
</Warning>
...
If required, set `displayedAttributes` back to its previous value using the [update displayed attributes endpoint](/reference/api/settings/displayed-attributes#update-displayed-attributes).
