...
Meilisearch will return a summarized task object and begin creating the dump. [Use the returned object's `taskUid` to monitor its progress.](/products/platform/tasks)
...
<Warning>
Meilisearch Cloud automatically generates a new master key during project creation. If you are using [security keys](/products/platform/basic_security), update your application so it uses the newly created Meilisearch Cloud API keys.
</Warning>
...
<CardGroup cols={2}>
  <Card title="API keys" icon="key" href="/products/platform/basic_security">
    Configure API keys for your new Cloud project
  </Card>
  <Card title="Cloud features" icon="cloud" href="/getting_started/overview">
    Explore features available in Meilisearch Cloud
  </Card>
</CardGroup>
