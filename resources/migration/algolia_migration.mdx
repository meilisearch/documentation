...
Create a Meilisearch client by passing the host URL and API key of your Meilisearch instance. The easiest option is to use the automatically generated [admin API key](/products/platform/basic_security).
...
To customize your index settings, we recommend following [this guide](/getting_started/glossary/indexes#index-settings). To learn more about the differences between settings in Algolia and Meilisearch, read on.
...
**In Meilisearch,** [index settings](/reference/api/settings/overview) and [search parameters](/reference/api/indexes/search-with-post#search-parameters) are two distinct categories. Settings affect all searches on an index, while parameters affect the results of a single search.
