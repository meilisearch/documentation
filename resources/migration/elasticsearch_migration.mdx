...
This guide walks you through exporting documents from an Elasticsearch index and importing them into Meilisearch using a [Node.js](https://nodejs.org/en/) script. [You can also skip directly to the finished script](#finished-script).
...
Before continuing, make sure you have both Meilisearch and Node.js installed and have access to a command-line terminal. If you're unsure how to install Meilisearch, see our [quick start](/resources/self_hosting/getting_started).

<Note>
This guide was tested with the following package versions:

- [`node.js`](https://nodejs.org/en/): `18` or later
- [`@elastic/elasticsearch`](https://www.npmjs.com/package/@elastic/elasticsearch): `8.x`
- [`meilisearch`](https://www.npmjs.com/package/meilisearch): compatible with Meilisearch v1.0 and above
</Note>
...
Start by creating a directory and generating a `package.json` file with `npm`:
...
This file will contain the migration script.

### Install dependencies

You need two packages: `@elastic/elasticsearch`, the official Elasticsearch JavaScript client, and `meilisearch`, the Meilisearch JavaScript client.
...
You need your Elasticsearch **host URL** and authentication credentials. Paste the below code in `script.js`:
...
function prepareDocuments(hits) {
  return hits.map((hit) => {
    ...
    // Use Elasticsearch's _id as the Meilisearch primary key
    // Meilisearch primary keys must only contain alphanumeric
...
Use the Meilisearch JavaScript method [`addDocumentsInBatches`](https://github.com/meilisearch/meilisearch-js#documents-) to upload all records in batches of 100,000.
...
To customize your index settings, see [configuring index settings](/learn/getting_started/indexes#index-settings). To understand the differences between Elasticsearch and Meilisearch settings, read on.
