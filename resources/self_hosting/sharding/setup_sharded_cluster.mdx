...
Send a single `PATCH /network` request to the leader instance (`ms-00`). The leader propagates the configuration to all other remotes automatically. You must define `remotes` and `shards` together in the same request:
