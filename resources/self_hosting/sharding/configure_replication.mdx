...
Replication ensures that your data exists on multiple remotes. However, Meilisearch does not currently provide automatic search failover between replicas. If a replica is unreachable during a network search, results from that shard may be missing from the response.
...
Send the full `remotes` list for each shard you want to update.
