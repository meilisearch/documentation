...
Use the task object's `taskUid` to [monitor the task status](/reference/api/tasks/get-task). The Hugging Face embedder will be ready to use once the task is completed.
