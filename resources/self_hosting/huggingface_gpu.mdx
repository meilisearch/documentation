---
title: Computing Hugging Face embeddings with GPU
sidebarTitle: GPU embeddings
description: This guide shows you how to compile a Meilisearch binary that generates Hugging Face embeddings with an Nvidia GPU.
---

This guide shows you how to compile a Meilisearch binary that generates Hugging Face embeddings with an Nvidia GPU.
...
Use the task object's `taskUid` to [monitor the task status](/reference/api/tasks/overview#get-one-task). The Hugging Face embedder will be ready to use once the task is completed.
