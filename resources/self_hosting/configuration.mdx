...
Meilisearch automatically collects data from all instances that do not opt out using this flag. All gathered data is used solely for the purpose of improving Meilisearch, and can be [deleted at any time](/resources/telemetry#how-to-delete-all-collected-data).

[Read more about our policy on data collection](/resources/internals/telemetry), or take a look at [the comprehensive list of all data points we collect](/resources/telemetry#exhaustive-list-of-all-collected-data).
...
Sets the maximum size of [accepted payloads](/resources/glossary/documents#dataset-format). Value must be given in bytes or explicitly stating a base unit. For example, the default value can be written as `107374182400`, `'107.7Gb'`, or `'107374 Mb'`.
...
The webhook payload contains the list of finished tasks in [ndjson](https://github.com/ndjson/ndjson-spec). For more information, [consult the dedicated task webhook guide](/products/platform/task_webhook).
