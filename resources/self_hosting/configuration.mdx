...
description: Configure your Meilisearch instance with command-line options, environment variables, or a configuration file.
sidebarDepth: 3
---
...
These startup options affect your entire Meilisearch instance, not just a single index. For settings that affect search within a single index, see [index settings](/reference/api/settings/overview).
...
To configure Meilisearch with command-line options in a cloud-hosted instance, edit its [service file](/resources/self_hosting/getting_started#step-4-run-meilisearch-as-a-service). The default location of the service file is `/etc/systemd/system/meilisearch.service`.
...
- Setting a [master key](/products/platform/basic_security) of at least 16 bytes is **mandatory**. If no master key is provided or if it is under 16 bytes, Meilisearch will suggest a secure autogenerated master key
- The [search preview interface](/resources/archives/search_preview) is disabled

`development`:

- Setting a [master key](/products/platform/basic_security) is **optional**. If no master key is provided or if it is under 16 bytes, Meilisearch will suggest a secure autogenerated master key
...
Sets the instance's master key, automatically protecting all routes except [`GET /health`](/reference/api/health/overview). This means you will need a valid API key to access all other endpoints.
...
[Learn more about Meilisearch's use of security keys.](/products/platform/basic_security)
...
Meilisearch automatically collects data from all instances that do not opt out using this flag. All gathered data is used solely for the purpose of improving Meilisearch, and can be [deleted at any time](/resources/internals/telemetry#how-to-delete-all-collected-data).

[Read more about our policy on data collection](/resources/internals/telemetry), or take a look at [the comprehensive list of all data points we collect](/resources/internals/telemetry#exhaustive-list-of-all-collected-data).
...
[Learn more about creating dumps](/reference/api/dumps/overview).
...
Sets the maximum size of [accepted payloads](/getting_started/glossary/documents#dataset-format). Value must be given in bytes or explicitly stating a base unit. For example, the default value can be written as `107374182400`, `'107.7Gb'`, or `'107374 Mb'`.
...
Notifies the configured URL whenever Meilisearch [finishes processing a task](/products/platform/asynchronous_operations#task-status) or batch of tasks. Meilisearch uses the URL as given, retaining any specified query parameters.

The webhook payload contains the list of finished tasks in [ndjson](https://github.com/ndjson/ndjson-spec). For more information, [consult the dedicated task webhook guide](/products/platform/webhooks).

The task webhook option requires having access to a command-line interface. If you are using Meilisearch Cloud, use the [`/webhooks` API route](/reference/api/webhooks/overview) instead.
