...
Sets the maximum size of [accepted payloads](/learn/getting_started/documents#dataset-format). Value must be given in bytes or explicitly stating a base unit. For example, the default value can be written as `100000000`, `'100Mb'`, or `'100 MB'`.
...
Enables the `CONTAINS` filter operator at launch. Can also be toggled at runtime via the `/experimental-features` API route.
...
Configure the number of concurrent search requests each CPU core can handle.

### Allowed IP networks <NoticeTag type="experimental" label="experimental" />
