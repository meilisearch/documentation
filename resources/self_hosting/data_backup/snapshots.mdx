...
After launch, Meilisearch waits for the configured interval before creating the first snapshot. You will find it in the [snapshot directory](/resources/self_hosting/configuration#snapshot-destination), `snapshots/`. Meilisearch will then create a new snapshot every 24 hours until you terminate your instance.
...
<Tip>
If you need to generate a single snapshot without relaunching your instance, use [the `/snapshots` route](/reference/api/management/create-snapshot).
</Tip>
...
You can change this behavior by specifying [`--ignore-snapshot-if-db-exists=true`](/resources/self_hosting/configuration#ignore-snapshot-if-db-exists). This will cause Meilisearch to launch with the existing database and ignore the snapshot without throwing an error.
</Warning>
