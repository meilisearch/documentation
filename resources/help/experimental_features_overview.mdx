...
| [Task and batch streams](/capabilities/indexing/tasks_and_batches/monitor_tasks#streaming-task-updates-instead-of-polling) | Subscribe to live task and batch updates over a stream instead of polling | API route |
| [MCP route](/getting_started/integrations/mcp)                              | Enable the built-in Model Context Protocol server for search-only AI agent access  | API route |
