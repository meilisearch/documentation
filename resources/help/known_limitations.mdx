...
This article covers hard limits that cannot be altered. Meilisearch also has some default limits that _can_ be changed, such as a [default payload limit of 100MB](/resources/self_hosting/configuration#payload-limit-size) and a [default search limit of 20 hits](/reference/api/indexes/search-with-post#limit).
...
**Explanation:** Handling large files may result in degraded user experience and performance issues. To add datasets larger than 20mb to a Meilisearch Cloud project, use the [add documents endpoint](/reference/api/documents/overview#add-or-replace-documents) or [`meilisearch-importer`](https://github.com/meilisearch/meilisearch-importer).

## Maximum number of query words

**Limitation:** The maximum number of terms taken into account for each [search query](/reference/api/indexes/search-with-post#query-q) is 10. If a search query includes more than 10 words, all words after the 10th will be ignored.
...
**Limitation:** searches using the [`filter` search parameter](/reference/api/indexes/search-with-post#filter) may have a maximum filtering depth of 2000.
...
**Explanation:** Meilisearch limits the maximum amount of returned search results to protect your database from malicious scraping. You may change this by using the `maxTotalHits` property of the [pagination index settings](/reference/api/settings/get-pagination#pagination-object). `maxTotalHits` only applies to the [search route](/reference/api/indexes/search-with-post) and has no effect on the [get documents with POST](/reference/api/documents/overview#get-documents-with-post) and [get documents with GET](/reference/api/documents/overview#get-documents-with-get) endpoints.
...
**Limitation:** When [searching for facet values](/reference/api/facet-search/overview), Meilisearch returns a maximum of 100 facets.
