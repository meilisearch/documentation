...
Meilisearch's keyword-based search relies on Charabia for tokenization, but [hybrid search](/learn/search/hybrid_search) and [semantic search](/learn/search/semantic_search) use embedding models that can handle languages independently of the tokenizer.
...
Yes. For keyword search, results may be less relevant than for fully optimized languages. However, you can use [hybrid search](/learn/search/hybrid_search) with a multilingual embedding model to get strong semantic results regardless of tokenization support.
