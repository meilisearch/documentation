...
The primary field is a special field that must be present in all documents. Its attribute is the [primary key](getting_started/primary_key#primary-field) and its value is the [document id](getting_started/primary_key#document-id). If you try to [index a document](/resources/self_hosting/getting_started#add-documents) that's missing a primary key or possessing the wrong primary key for a given index, it will cause an error and no documents will be added.

To learn more, refer to the [primary key explanation](getting_started/primary_key).
