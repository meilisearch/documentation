...
If the value is a string, it **[can contain at most 65535 positions](resources/known_limitations#maximum-number-of-words-per-attribute)**. Words exceeding the 65535 position limit will be ignored.
...
<Note>
Some features require Meilisearch to reserve attributes. For example, to use [geosearch functionality](/products/geo_search/geosearch) your documents must include a `_geo` field.
...
You can modify this behavior using the [update settings endpoint](/reference/api/settings#update-settings), or the respective update endpoints for [displayed attributes](/reference/api/settings/displayed-attributes#update-displayed-attributes), and [searchable attributes](/reference/api/settings/searchable-attributes#update-searchable-attributes) so that a field is:
...
The primary field is a special field that must be present in all documents. Its attribute is the [primary key](resources/glossary/primary_key#primary-field) and its value is the [document id](resources/glossary/primary_key#document-id). If you try to [index a document](/resources/self_hosting/getting_started#add-documents) that's missing a primary key or possessing the wrong primary key for a given index, it will cause an error and no documents will be added.

To learn more, refer to the [primary key explanation](resources/glossary/primary_key).
...
Meilisearch uses a lot of RAM when indexing documents. Be aware of your [RAM availability](resources/faq#what-are-the-recommended-requirements-for-hosting-a-meilisearch-instance) as you increase your batch size as this could cause Meilisearch to crash.
