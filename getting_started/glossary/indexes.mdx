...
You can explicitly create an index using the [create index endpoint](/reference/api/indexes/overview#create-an-index). Once created, you can add documents using the [add documents endpoint](/reference/api/documents/overview#add-or-update-documents).
...
You can change an index's `uid` using the [`/indexes` API route](/reference/api/indexes/overview#update-an-index).
...
You can set the primary key for an index or let it be inferred by Meilisearch. Read more about [setting the primary key](/getting_started/glossary/primary_key#setting-the-primary-key).

[Learn more about the primary field](/getting_started/glossary/primary_key)

## Index settings

Index settings can be thought of as a JSON object containing many different options for customizing search behavior.

To change index settings, use the [update settings endpoint](/reference/api/settings/overview#update-settings) or any of the child routes.
...
You can update these field attributes using the [update settings endpoint](/reference/api/settings/overview#update-settings), or the respective endpoints for [displayed attributes](/reference/api/settings/get-displayedattributes#update-displayed-attributes) and [searchable attributes](/reference/api/settings/get-searchableattributes#update-searchable-attributes).

[Learn more about displayed and searchable attributes.](/products/full_text_search/displayed_searchable_attributes)
...
Designate the distinct attribute using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update distinct attribute endpoint](/reference/api/settings/get-distinctattribute#update-distinct-attribute). **You can only set one field as the distinct attribute per index.**

[Learn more about distinct attributes.](/products/full_text_search/distinct_attribute)

### Faceting

Facets are a specific use-case of filters in Meilisearch: whether something is a facet or filter depends on your UI and UX design. Like filters, you need to add your facets to [`filterableAttributes`](/reference/api/settings/get-filterableattributes#update-filterable-attributes), then make a search query using the [`filter` search parameter](/reference/api/indexes/search-with-post#filter).

By default, Meilisearch returns `100` facet values for each faceted field. You can change this using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update faceting settings endpoint](/reference/api/settings/get-faceting#update-faceting-settings).

[Learn more about faceting.](/products/faceted_search/search_with_facet_filters)
...
Before filtering on any document attribute, you must add it to `filterableAttributes` using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update filterable attributes endpoint](/reference/api/settings/get-filterableattributes#update-filterable-attributes). Then, make a search query using the [`filter` search parameter](/reference/api/indexes/search-with-post#filter).

[Learn more about filtering.](/products/faceted_search/filter_search_results)

### Pagination

To protect your database from malicious scraping, Meilisearch only returns up to `1000` results for a search query. You can change this limit using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update pagination settings endpoint](/reference/api/settings/get-pagination#update-pagination-settings).

[Learn more about pagination.](/guides/performance/pagination)
...
You can alter this order or define custom ranking rules to return certain results first. This can be done using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update ranking rules endpoint](/reference/api/settings/get-rankingrules#update-ranking-rules).

[Learn more about ranking rules.](/products/full_text_search/ranking_rules)
...
Add the attributes you'd like to sort by to `sortableAttributes` using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update sortable attributes endpoint](/reference/api/settings/get-sortableattributes#update-sortable-attributes). You can then use the [`sort` search parameter](/reference/api/indexes/search-with-post#sort) to sort your results in ascending or descending order.

[Learn more about sorting.](/products/faceted_search/sort_search_results)
...
Change your index's stop words list using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update stop words endpoint](/reference/api/settings/get-stopwords#update-stop-words). In addition to improving relevancy, designating common words as stop words greatly improves performance.
...
Since synonyms are defined for a given index, they won't apply to any other index on the same Meilisearch instance. You can create your list of synonyms using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update synonyms endpoint](/reference/api/settings/get-synonyms#update-synonyms).

[Learn more about synonyms.](/products/full_text_search/synonyms)
...
You can update the typo tolerance settings using the [update settings endpoint](/reference/api/settings/overview#update-settings) or the [update typo tolerance endpoint](/reference/api/settings/get-typotolerance#update-typo-tolerance-settings).

[Learn more about typo tolerance.](/products/full_text_search/typo_tolerance_settings)
...
For more information, see the [swap indexes endpoint](/reference/api/indexes/overview#swap-indexes).
