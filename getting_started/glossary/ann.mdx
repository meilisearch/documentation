...
**Approximate Nearest Neighbors (ANN)** is an algorithm for finding vectors similar to a query vector. It's the foundation of [vector search](/resources/glossary/vector_search) and powers Meilisearch's AI-powered search features.

## The problem

Given a query vector and millions of stored vectors, find the most similar ones.

**Exact search** compares the query against every vector. For 1 million 768-dimension vectors, that's 768 million floating-point operations per search—too slow for real-time applications.
...
- [Vector search](/resources/glossary/vector_search): Using ANN for semantic search
- [Vector embeddings](/resources/glossary/vector_embeddings): The vectors being searched
- [Hybrid search](/resources/glossary/hybrid_search): Combining ANN with keyword search
