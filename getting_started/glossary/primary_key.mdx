...
An [index](/getting_started/glossary/indexes) in Meilisearch is a collection of [documents](/getting_started/glossary/documents). Documents are composed of fields, each field containing an attribute and a value.
...
The primary key cannot be changed while documents are present in the index. To change the primary key of an index that already contains documents, you must therefore [delete all documents](/reference/api/documents/overview#delete-all-documents) from that index, [change the primary key](/reference/api/indexes/overview#update-an-index), then [add them](/reference/api/documents/overview#add-or-replace-documents) again.
