...
First, create a new Meilisearch project on Meilisearch Cloud. You can also [install and run Meilisearch locally or in another cloud service](/resources/self_hosting/getting_started#setup-and-installation).
...
_More about [Meilisearch security](/products/platform/basic_security)._
...
<Warning>
Since the configuration file is public, we strongly recommend providing a key that can only access [the search endpoint](/reference/api/indexes/search-with-post) , such as the `Default Search API Key`, in a production environment.
Read more about [Meilisearch security](/products/platform/basic_security).
</Warning>
...
You can check [this tutorial](/resources/self_hosting/getting_started) if you now want to run Meilisearch in production!
