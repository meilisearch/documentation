...
The API key should have only the permissions required to add documents into your Meilisearch instance. In production, prefer a dedicated scoped key for scraping/indexing operations instead of the `Default Admin API Key`.
